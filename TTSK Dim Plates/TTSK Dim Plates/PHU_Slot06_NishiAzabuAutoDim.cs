...
namespace Tekla.Technology.Akit.UserScript
{
    ...
    internal static class PHU_NishiAzabuDimensionEngine
    {
        ...
        private sealed class GeometryGroup
        {
            ...
            public bool IsValid
            {
                get { return Parts.Count > 0 && IsFinite(MinX); }
            }
        ...
        internal static string RunSlot07()
        {
            ...
            try
            {
                ...
                List<TSD.DrawingObject> backgroundDimensions = CollectBackgroundDimensions(
                    context.Views
                );
                ...
                return "Tạo " + created.Count + " dim trên 4 view, Xóa " + deleted + " dim nền";
            }
            catch (Exception ex)
            {
        ...
        internal static string AuditPlanSlot07()
        {
            try
            {
                ...
                text.AppendLine(
                    "BackgroundDimensionCount=" + CollectBackgroundDimensions(context.Views).Count
                );
                foreach (KeyValuePair<string, ViewGeometry> pair in context.Views)
                {
                    text.Append(pair.Key)
                        .Append(" scale=")
                        .Append(Format(pair.Value.Scale))
                        .Append(" parts=")
                        .Append(pair.Value.Parts.Count)
                        .AppendLine();
                }
                for (int i = 0; i < plans.Count; i++)
                {
                    ...
                    text.Append(plan.Name)
                        ...
                        .Append(plan.Side)
                        .Append(" line=")
                        .Append(Format(plan.LineCoordinate))
                        .Append(" distance=")
                        .Append(Format(GetDistance(plan)))
                        .Append(" combine=")
                        .Append(
        ...
        private static Context AnalyzeDrawing()
        {
            ...
            if (
                !context.Model.GetConnectionStatus()
                || !context.DrawingHandler.GetConnectionStatus()
            )
                throw new InvalidOperationException("Không kết nối được Tekla Model/Drawing API.");
            ...
            TSM.TransformationPlane currentPlane = context
        ...
        )
        {
            ...
            ReadGridReferences(model, view, result, currentToGlobal, globalToView);
        ...
        )
        {
            try
            {
                TSD.DrawingObjectEnumerator gridLines = view.GetAllObjects(typeof(TSD.GridLine));
                HashSet<int> seen = new HashSet<int>();
                while (gridLines != null && gridLines.MoveNext())
                {
                    TSD.GridLine drawingGridLine = gridLines.Current as TSD.GridLine;
                    if (
                        drawingGridLine == null
                        || drawingGridLine.ModelIdentifier == null
                        || !seen.Add(drawingGridLine.ModelIdentifier.ID)
                    )
                        continue;

                    TSM.GridPlane gridPlane =
                        model.SelectModelObject(drawingGridLine.ModelIdentifier) as TSM.GridPlane;
                    if (
                        gridPlane == null
                    ...
                    P2 projectedOrigin = Transform(origin, currentToGlobal, globalToView);
                    ...
                    P2 lineVector =
                        Distance(projectedOrigin, projectedX)
                        >= Distance(projectedOrigin, projectedY)
                            ? Pt(projectedX.X - projectedOrigin.X, projectedX.Y - projectedOrigin.Y)
                            : Pt(
                    ...
                    {
                        AddUniqueCoordinate(result.VerticalGridCoordinates, projectedOrigin.X);
                    }
                    else if (Math.Abs(lineVector.X) > Math.Abs(lineVector.Y) * 10.0)
                    {
                        AddUniqueCoordinate(result.HorizontalGridCoordinates, projectedOrigin.Y);
                    }
                }
            }
            catch
        ...
        {
            return new TSG.Point(point.X + vector.X, point.Y + vector.Y, point.Z + vector.Z);
        }
        ...
        )
        {
            try
            {
                ...
                while (edges != null && edges.MoveNext())
                {
                    ...
                    projected.AddVertex(Transform(edge.StartPoint, currentToGlobal, globalToView));
                    projected.AddVertex(Transform(edge.EndPoint, currentToGlobal, globalToView));
                }
            }
            catch { }
        }
            ...
            try
            {
                ...
                while (bolts != null && bolts.MoveNext())
                {
                    ...
                    foreach (object value in group.BoltPositions)
                    {
                        ...
                        if (point != null)
                        {
                            projected.AddBolt(Transform(point, currentToGlobal, globalToView));
                        }
                    }
                }
            }
        ...
        private static void BuildViewA(ViewGeometry view, List<DimPlan> plans, List<string> errors)
        {
            ...
            if (!RequireGroups(view, errors, concrete, fp, wp, splice))
                return;
            ...
            AddPlan(plans, view, "A-D09", DimSide.Bottom, internalBottom, rightBolt, fpRB);
            AddPlan(plans, view, "A-D10", DimSide.Bottom, internalBottom, leftBolt, fpLB);

            double gridX;
            if (
                TryGetClosestCoordinate(view.VerticalGridCoordinates, wp.CenterX, out gridX)
        ...
        private static void BuildViewB(ViewGeometry view, List<DimPlan> plans, List<string> errors)
        {
            GeometryGroup main = Group(view.Main);
            GeometryGroup concrete = ConcreteGroup(view);
            ...
            if (!RequireGroups(view, errors, concrete, fill, splice, rp, wp, dummy45))
                return;
            ...
            AddPlan(
                ...
                Pt(fill.MaxX, dummy45.MaxY),
                Pt(fill.MaxX, fill.MinY)
            );
            AddPlan(
                plans,
                view,
        ...
        private static void BuildViewC(ViewGeometry view, List<DimPlan> plans, List<string> errors)
        {
            GeometryGroup main = Group(view.Main);
            GeometryGroup concrete = ConcreteGroup(view);
            GeometryGroup wp = PositionGroup(view, "CUWP");
            ...
            if (
                !RequireGroups(view, errors, concrete, wp)
                || rpParts.Count < 2
                || fpParts.Count < 2
                || uaParts.Count < 2
            )
            ...
            AddPlan(
                ...
                Pt(main.MinX, concrete.MinY),
                mainLB,
                mainLT,
                Pt(main.MinX, concrete.MaxY)
            );
            AddPlan(plans, view, "C-D04", DimSide.Top, main.MaxY + 4.24923 * step, mainLT, mainRT);
            AddPlan(
                plans,
                view,
                "C-D05",
            ...
            double gridY;
            if (
                TryGetClosestCoordinate(view.HorizontalGridCoordinates, wp.CenterY, out gridY)
        ...
        )
        {
            ...
            List<ProjectedPart> fpParts = PositionParts(view, "CUFP");
            if (
                !RequireGroups(view, errors, concrete, wp, spliceLeft, spliceRight)
                || rpParts.Count < 2
                || fpParts.Count < 2
            )
            {
            ...
            if (
                ...
                || rpRightOuter == null
                || rpLeftBottom == null
                || mainBoltsTop.Count < 4
                || spliceLeftBolts.Count < 2
                || spliceRightBolts.Count < 2
            )
            {
            ...
            AddPlan(
                ...
                rpLeftBottom,
                mainLT,
                Pt(main.MinX - 0.04 * concrete.Width, concrete.MinY)
            );
            AddPlan(plans, view, "F-D02", DimSide.Top, main.MaxY + 4.34440 * step, mainLT, mainRT);
            AddPlan(
                plans,
                view,
                "F-D03",
        ...
        )
        {
            ...
            AddPlan(
                ...
                topBolts[0],
                topBolts[1],
                topBolts[topBolts.Count - 2],
                topBolts[topBolts.Count - 1],
                mainRT
            );
        ...
        )
        {
            ...
            AddPlan(
                ...
                Pt(concrete.MinX, concrete.MaxY),
                mainLB,
                Pt(wp.MinX, wp.MaxY)
            );
            AddPlan(
        ...
        )
        {
            ...
            if (
                !RequireGroups(view, errors, wp, dummy28, dummy25Right)
                || rpParts.Count < 2
                || sp2Columns.Count != 4
            )
            {
            ...
            P2 dummyBoltLeft = ClosestBoltByXSlot07(dummy28.Bolts, wpBolts[1].X, true);
            P2 dummyBoltRight = ClosestBoltByXSlot07(
                dummy28.Bolts,
                wpBolts[wpBolts.Count - 2].X,
                true
            );
            ...
            fD01.Add(mainRB);
            AddPlanList(plans, view, "S07-F-D01", DimSide.Bottom, main.MinY - 3.96074 * step, fD01);
            ...
            AddPlanList(plans, view, "S07-F-D06", DimSide.Top, wp.MaxY + 4.97557 * step, fD06);
            ...
            DimSide fD10Side = fD10Line >= rpLeftOuter.Y ? DimSide.Top : DimSide.Bottom;
            AddPlan(plans, view, "S07-F-D10", fD10Side, fD10Line, rpLeftOuter, mainLT);
            AddPlan(
                plans,
                view,
                "S07-F-D11",
        ...
        private static void ValidatePlanGeometrySlot07(DimPlan plan)
        {
            ...
            for (int i = 0; i < plan.Points.Count; i++)
            {
                ...
                double value =
                    plan.Side == DimSide.Left || plan.Side == DimSide.Right ? point.Y : point.X;
                minimum = Math.Min(minimum, value);
                maximum = Math.Max(maximum, value);
            }

            if (maximum - minimum <= GeometryTolerance || GetDistance(plan) <= GeometryTolerance)
                throw new InvalidOperationException(
        ...
        private static void ValidateReady(Context context, List<DimPlan> plans)
        {
            ...
            int maximumPlanCount = BaseDimensionSetCount + MaximumOffsetDimensionCount;
            if (
                plans == null
                || plans.Count < BaseDimensionSetCount
                || plans.Count > maximumPlanCount
            )
                throw new InvalidOperationException(
                    "Preflight phải tạo từ "
                        + BaseDimensionSetCount
                        + " đến "
                        + maximumPlanCount
                        + " plan, hiện có "
                        + (plans == null ? 0 : plans.Count)
            ...
            for (int i = 0; i < plans.Count; i++)
            {
                ...
                if (maxMeasure - minMeasure <= GeometryTolerance)
                    throw new InvalidOperationException(plan.Name + " không có khoảng đo hợp lệ.");
        ...
        )
        {
            ...
            foreach (KeyValuePair<string, ViewGeometry> pair in views)
            {
                ...
                while (all != null && all.MoveNext())
                {
                    ...
                    bool isSet =
                        name.IndexOf("DimensionSet", StringComparison.OrdinalIgnoreCase) >= 0;
                    bool isStandalone =
        ...
        private static TSG.Vector GetDirection(DimSide side)
        {
            if (side == DimSide.Left)
                return new TSG.Vector(-1.0, 0.0, 0.0);
        ...
        )
        {
            ...
            if (!combineDisabled)
                throw new InvalidOperationException(
                    "Tekla đã ghi đè kiểu hiển thị của "
                        + plan.Name
        ...
        private static double GetDistance(DimPlan plan)
        {
            ...
            if (plan.Side == DimSide.Left)
                return first.X - plan.LineCoordinate;
            if (plan.Side == DimSide.Right)
                return plan.LineCoordinate - first.X;
            if (plan.Side == DimSide.Top)
                return plan.LineCoordinate - first.Y;
        ...
        private static List<ProjectedPart> RolePartsSlot07(ViewGeometry view, string role)
        {
            ...
            if (marker == "SP-1" || marker == "SP-2")
            {
                ...
                splice.Sort(
                    delegate(ProjectedPart a, ProjectedPart b)
                    {
                ...
                for (int i = 0; i < splice.Count; i++)
                {
                    ...
                    if ((marker == "SP-1" && inPair) || (marker == "SP-2" && !inPair))
        ...
        private static GeometryGroup RoleGroupSlot07(ViewGeometry view, string role)
        {
        ...
        private static List<GeometryGroup> RoleCenterGroupsSlot07(ViewGeometry view, string role)
        {
            List<ProjectedPart> parts = RolePartsSlot07(view, role);
            parts.Sort(
                delegate(ProjectedPart a, ProjectedPart b)
            ...
            groups.Sort(
                delegate(GeometryGroup a, GeometryGroup b)
                {
                    return a.CenterX.CompareTo(b.CenterX);
                }
            );
        ...
        private static GeometryGroup ShortConcreteGroupSlot07(ViewGeometry view)
        {
            ...
            for (int i = 0; i < view.Parts.Count; i++)
            {
                ProjectedPart part = view.Parts[i];
                bool dummy =
                    part.Name.IndexOf("DUMMY", StringComparison.Ordinal) >= 0
        ...
        private static GeometryGroup DummyPlateGroupSlot07(ViewGeometry view, double thickness)
        {
            GeometryGroup result = new GeometryGroup();
        ...
        )
        {
            ...
            double target = minimumX ? Double.PositiveInfinity : Double.NegativeInfinity;
            for (int i = 0; i < parts.Count; i++)
            {
                target = minimumX
        ...
        private static P2 TopVertexSlot07(List<P2> points, bool minimumXOnTie)
        {
            ...
            for (int i = 0; i < points.Count; i++)
            {
                ...
                if (best == null || point.Y > best.Y + GeometryTolerance)
                {
                    ...
                    && ((minimumXOnTie && point.X < best.X) || (!minimumXOnTie && point.X > best.X))
                )
                {
                    best = point;
                }
        ...
        private static P2 ExtremeXVertexSlot07(List<P2> points, bool minimumX, bool maximumYOnTie)
        {
            P2 best = null;
            for (int i = 0; i < points.Count; i++)
            {
                ...
                )
                {
                    ...
                    Math.Abs(point.X - best.X) <= GeometryTolerance
                    && ((maximumYOnTie && point.Y > best.Y) || (!maximumYOnTie && point.Y < best.Y))
                )
                {
                    best = point;
                }
        ...
        private static P2 ClosestBoltByXSlot07(List<P2> bolts, double x, bool maximumYOnTie)
        {
            P2 best = null;
            double bestDistance = Double.PositiveInfinity;
        ...
        private static List<ProjectedPart> PositionParts(ViewGeometry view, string token)
        {
            ...
            return result.Count > 0 ? result : GeometricRoleParts(view, upper);
        }

        private static List<ProjectedPart> GeometricRoleParts(ViewGeometry view, string role)
        {
            ...
            bool takeNarrowPair =
                (view.Key == "A" || view.Key == "B") ? role == "CURP" : role == "CUFP";
        ...
        private static bool IsPlateProfile(ProjectedPart part, double thickness)
        {
            if (
                part == null
                || String.IsNullOrEmpty(part.Profile)
        ...
        )
        {
            ...
            for (int i = 0; i < view.Parts.Count; i++)
            {
                ProjectedPart part = view.Parts[i];
                if (
                    part.Name.IndexOf(SafeUpper(nameToken), StringComparison.Ordinal) >= 0
        ...
        private static ProjectedPart ExtremePart(List<ProjectedPart> parts, bool minimumX)
        {
        ...
        private static P2 ExtremeVertex(List<P2> points, bool minimumX, bool minimumYOnTie)
        {
            P2 best = null;
            for (int i = 0; i < points.Count; i++)
            {
                ...
                else if (Math.Abs(delta) <= GeometryTolerance && minimumYOnTie && point.Y < best.Y)
                {
                    best = point;
                }
            }
        ...
        private static P2 BottomVertex(List<P2> points, bool minimumXOnTie)
        {
            ...
            for (int i = 0; i < points.Count; i++)
            {
                ...
                if (
                    best == null
                    || point.Y < best.Y - GeometryTolerance
                    || (
                        Math.Abs(point.Y - best.Y) <= GeometryTolerance
                        && (
        ...
        private static P2 ExtremeBolt(List<P2> bolts, bool minimumX, bool minimumYOnTie)
        {
        ...
        private static string GetViewKey(TSD.View view)
        {
            ...
            string name = SafeUpper(
                Convert.ToString(GetMember(view, "Name"), CultureInfo.InvariantCulture)
            );
        ...
        {
            MessageBox.Show(message, "Auto Dimension - Nishi Azabu", MessageBoxButtons.OK, icon);
        }
    }
}
