...
#pragma warning restore 618
        ...
        private sealed class HoleMarkLayoutItemV3
        {
            ...
            public HoleMarkCandidateV3 Selected;
        }
        ...
        )
        {
            ...
            foreach (Mark mark in holeMarks)
            {
                try
                {
                    if (!(mark.Placing is LeaderLinePlacing) || mark.Attributes == null)
                        continue;

                    Mark.MarkAttributes attributes = mark.Attributes;
                    attributes.PreferredPlacing = new LeaderLinePlacingType();
                    if (attributes.PlacingAttributes != null)
                        attributes.PlacingAttributes.IsFixed = true;
            ...
            foreach (Mark mark in holeMarks)
            {
                try
                {
                    ...
                    item.Anchors = ResolveHoleMarkAnchorCandidatesV3(
                        item.CurrentAnchor,
                        holeGroups,
            ...
            foreach (HoleMarkLayoutItemV3 item in items)
            {
                ...
                try
                {
                    item.Mark.Placing = new LeaderLinePlacing(
                        new Point(candidate.Anchor.X, candidate.Anchor.Y, 0)
                    );
                    item.Mark.InsertionPoint = new Point(
                        candidate.Center.X,
                        candidate.Center.Y,
                        0
                    );
            ...
            // Mark box có thể đổi chiều rộng sau lần Modify đầu (font/ký hiệu
            // HOLE được Tekla layout lại). Đọc box thật và dịch mark để điểm
            // tiếp xúc đã chọn vẫn giữ đúng góc, ngay trong một lần chạy.
            // Tối đa 5 pass vì Tekla đôi khi co box theo 2-3 bước liên tiếp
            // khi chuyển mark tự do sang fixed LeaderLinePlacing.
            for (int correctionPass = 0; correctionPass < 5; correctionPass++)
            {
                bool correctedAny = false;
                foreach (HoleMarkLayoutItemV3 item in items)
                {
                    if (item == null || item.Mark == null || item.Selected == null)
                        continue;
                    ...
                    try
                    {
                        ...
                        Point actualMin;
                        Point actualMax;
                        if (!TryGetObjectBox(item.Mark, out actualMin, out actualMax))
                            continue;

                        HoleMarkCandidateV3 selected = item.Selected;
                        Point actualContact = new Point(
                            ClampHoleMarkValueV3(
                                selected.Anchor.X,
                                Math.Min(actualMin.X, actualMax.X),
                                Math.Max(actualMin.X, actualMax.X)
                            ),
                            ClampHoleMarkValueV3(
                                selected.Anchor.Y,
                                Math.Min(actualMin.Y, actualMax.Y),
                                Math.Max(actualMin.Y, actualMax.Y)
                            ),
                            0
                        );
                        double correctionX = selected.Contact.X - actualContact.X;
                        double correctionY = selected.Contact.Y - actualContact.Y;
                        if (
                            Math.Abs(correctionX) <= 0.02
                            && Math.Abs(correctionY) <= 0.02
                        )
                        {
                            continue;
                        }

                        Point insertion = item.Mark.InsertionPoint;
                        if (insertion == null)
                            continue;
                        item.Mark.InsertionPoint = new Point(
                            insertion.X + correctionX,
                            insertion.Y + correctionY,
                            insertion.Z
                        );
                        if (item.Mark.Modify())
                            correctedAny = true;
        ...
        )
        {
            ...
            foreach (Point anchor in item.Anchors)
            {
                ...
                // Thứ tự chỉ dùng để phá hòa: trên, trái, phải, dưới.
                // Chiều dài leader thực tế luôn là tiêu chí chính.
        ...
        )
        {
            ...
            if (
                ...
                candidate.VisualTier = touchesDimension ? 2 : 0;
            else if (
                candidate.AngleDeviation
                <= HOLE_MARK_PREFERRED_ANGLE_TOL_DEG + angleTierTolerance
            )
                candidate.VisualTier = touchesDimension ? 3 : 1;
            else if (
                candidate.AngleDeviation
                <= HOLE_MARK_OBLIQUE_ANGLE_TOL_DEG + angleTierTolerance
            )
                candidate.VisualTier = touchesDimension ? 5 : 4;
            else
                candidate.VisualTier = touchesDimension ? 7 : 6;

            // Phân tầng thẩm mỹ là tiêu chí đầu tiên. Vì vậy leader ngang/dọc
            // ngắn tuyệt đối không thể thắng một leader xiên hợp lý. Trong cùng
            // tầng mới cân bằng: gần 45°, ít chạm DIM và chiều dài thực.
