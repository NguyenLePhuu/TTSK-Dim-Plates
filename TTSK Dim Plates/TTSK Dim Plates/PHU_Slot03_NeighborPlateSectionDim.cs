...
namespace Tekla.Technology.Akit.UserScript
{
    ...
    public class PHU_Slot03_NeighborPlateSectionDim
    {
        ...
        )
        {
            ...
            DrawingPart firstPlate =
                result.PlateDrawingParts.Count > 0 ? result.PlateDrawingParts[0] : null;
            DrawingPart firstNeighbor =
                result.NeighborDrawingParts.Count > 0 ? result.NeighborDrawingParts[0] : null;

            result.View = TryGetSelectedPartsView(
                firstPlate,
                result.MainDrawingPart,
                firstNeighbor
            );
        ...
        )
        {
            ...
            for (int i = 0; i < selectedParts.Count; i++)
            {
                ...
                if (
                    SameIdentifier(part.Identifier, cleanContext.MainBeam.Identifier)
        ...
        )
        {
            ...
            try
            {
                ...
                TSD.DrawingObjectEnumerator parts = context.View.GetAllObjects(typeof(DrawingPart));
        ...
        )
        {
            if (
                ...
                || context == null
                || context.View == null
                || context.MainBeam == null
                || context.Plates == null
                || context.Plates.Count == 0
            )
                return 0;
        ...
        )
        {
            ...
            try
            {
                ...
                Point mainRef = new Point((mainBox.MinX + mainBox.MaxX) / 2.0, mainBox.MaxY, 0);
                ...
                if (left != null && left.Valid)
                {
                    ...
                    else if (left.HolePoint != null && left.NeighborTopPoint != null)
                    {
                        if (
                            CreateDimChain(
                                ...
                                new Point[]
                                {
                                    left.NeighborTopPoint,
                                    GetHolePointWithPhiGap(
                                        left.Plate,
                                        left.Neighbor,
                                        left.HolePoint,
                                        new Vector(-1, 0, 0)
                    ...
                    if (left.MainToNeighborP1 != null && left.MainToNeighborP2 != null)
                    {
                        Point[] leftMainToNeighborChain =
                            left.NeighborTopPoint != null
                            && Math.Abs(left.MainToNeighborP2.Y - left.NeighborTopPoint.Y) > TOL
                                ? new Point[]
                                {
                                    left.MainToNeighborP2,
                                    left.NeighborTopPoint,
                                    left.MainToNeighborP1
                                }
                ...
                if (right != null && right.Valid)
                {
                    ...
                    if (right.MainToNeighborP1 != null && right.MainToNeighborP2 != null)
                    {
                        Point[] rightMainToNeighborChain =
                            right.NeighborTopPoint != null
                            && Math.Abs(right.MainToNeighborP2.Y - right.NeighborTopPoint.Y) > TOL
                                ? new Point[]
                                {
                                    right.MainToNeighborP2,
                                    right.NeighborTopPoint,
                                    right.MainToNeighborP1
                                }
        ...
        )
        {
            if (
                ...
                || !group.Valid
                || direction == null
                || group.HolePoints == null
                || group.HolePoints.Count == 0
            )
                return 0;
            ...
            {
                chain.Add(GetHolePointWithPhiGap(group.Plate, group.Neighbor, holes[i], direction));
            }
        ...
        )
        {
            ...
            try
            {
                ...
                SplitNeighborBeamsByOrientation(
                    mainBox,
                    neighborBeams,
                    horizontalNeighbors,
                    verticalNeighbors
            ...
            if (horizontalNeighbors.Count > 0)
                count += CreateSectionNeighborPlateDims(
                    model,
                    dh,
                    view,
                    mainBeam,
            ...
            if (verticalNeighbors.Count > 0)
                count += CreateSectionVerticalNeighborPlateDims(
                    model,
                    dh,
                    view,
                    mainBeam,
        ...
        )
        {
            ...
            try
            {
                ...
                VerticalGroup top = BuildVerticalGroup(
                    model,
                    view,
                    true,
                    mainBox,
                ...
                VerticalGroup bottom = BuildVerticalGroup(
                    model,
                    view,
                    false,
                    mainBox,
                ...
                if (top != null && top.Valid)
                    count += CreateOneVerticalNeighborGroupDims(
                        handler,
                        view,
                        tierManager,
                        mainBox,
        ...
        )
        {
            ...
            List<Point> linkedBoltCenters = GetLinkedPlateNeighborBoltCenters(
                model,
                view,
                plate,
                neighbor,
            ...
            {
                linkedBoltCenters = GetViewBoltCentersInPlateNeighborOverlap(
                    model,
                    view,
                    plateBox,
                    neighborBox
            ...
            bool holeIsBelowMainBottom = IsSlot03HoleBelowMainBottom(verticalHole.Y, mainBox.MinY);
            bool holeIsOnRightOfMain = IsSlot03HoleOnRightOfMain(verticalHole.X, mainCenterX);
            ...
            double horizontalPartEdgeY = horizontalDimOnTop ? mainBox.MaxY : mainBox.MinY;
            ...
            Point horizontalMainCenterEdgePoint = new Point(mainCenterX, horizontalPartEdgeY, 0);
        ...
        private static bool IsSlot03HoleBelowMainBottom(double holeY, double mainBottomY)
        {
        ...
        private static bool IsSlot03HoleOnRightOfMain(double holeX, double mainCenterX)
        {
        ...
        )
        {
            ...
            for (int i = 0; i < boltCenters.Count; i++)
            {
                ...
                if (
                    p.X < plateBox.MinX - 20.0
                    || p.X > plateBox.MaxX + 20.0
                    || p.Y < plateBox.MinY - 20.0
                    || p.Y > plateBox.MaxY + 20.0
                )
                    continue;
        ...
        private static void AddBoltGroupPoints(TSM.Model model, Identifier id, List<Point> result)
        {
            try
            {
        ...
        )
        {
            ...
            for (int i = 0; i < beams.Count; i++)
            {
                ...
                for (int p = 0; p < plates.Count; p++)
                {
                    ...
                    if (
                        !string.IsNullOrEmpty(plateAssembly)
            ...
            if (bestIndex < 0 || bestScore <= 0)
            {
                ...
                for (int i = 0; i < beams.Count; i++)
                {
                    ...
                    double area = box.Valid
                        ? Math.Abs(box.MaxX - box.MinX) * Math.Abs(box.MaxY - box.MinY)
                        : 0.0;
                    if (area > bestArea)
                    {
                        bestArea = area;
        ...
        private static ModelPart GetActiveAssemblyDrawingMainPart(
            TSM.Model model,
            TSD.Drawing drawing
        )
        {
            try
            {
                ...
                Identifier assemblyId = GetIdentifierProperty(
                    assemblyDrawing,
                    "AssemblyIdentifier"
                );
                if (assemblyId == null)
        ...
        private static bool PartBoltCollectionContainsDirectConnection(
            ModelPart owner,
            ModelPart other
        )
        {
        ...
        private static Point GetHolePointWithPhiGap(
            ModelPart plate,
            ModelPart neighbor,
            Point holeCenter,
            Vector direction
        ...
        )
        {
            ...
            try
            {
                ...
                if (left != null && left.Valid && left.Holes != null && left.Holes.Count > 0)
                {
                    ...
                    if (h != null)
                        horizontalChain.Add(
                            GetHolePointWithPhiGap(left.Plate, mainBeam, h, new Vector(0, 1, 0))
                        );
                }
                ...
                if (right != null && right.Valid && right.Holes != null && right.Holes.Count > 0)
                {
                    ...
                    if (h != null)
                        horizontalChain.Add(
                            GetHolePointWithPhiGap(right.Plate, mainBeam, h, new Vector(0, 1, 0))
                        );
                }
        ...
        private static List<Point> GetAllowedBoltCentersFromPlate(
            ModelPart plate,
            Bounds2D plateBox
        )
        {
        ...
        )
        {
            ...
            for (int i = 0; i < holes.Count; i++)
            {
                ...
                if (
                    p.X < plateBox.MinX - 25.0
                    || p.X > plateBox.MaxX + 25.0
                    || p.Y < plateBox.MinY - 25.0
                    || p.Y > plateBox.MaxY + 25.0
                )
                    continue;
        ...
        )
        {
            ...
            if (
                CreateDimChain(
                    ...
                    (
                        direction.X < 0.0
                            ? tierManager.TakeLeftDistance(holeChain[0], g.PlateBox)
                            : tierManager.TakeRightDistance(holeChain[0], g.PlateBox)
                    ),
                    "GEO_DIMENSION"
                )
            )
            ...
            if (
                CreateDimChain(
                    ...
                    (
                        direction.X < 0.0
                            ? tierManager.TakeLeftDistance(mainTop, g.PlateBox)
                            : tierManager.TakeRightDistance(mainTop, g.PlateBox)
                    ),
                    "GEO_DIMENSION"
                )
            )
        ...
        private static bool IsAllowedDimensionBoltGroup(ModelBoltGroup bg)
        {
            try
            {
                ...
                if (
                    type.IndexOf("GALVA") >= 0
                    || name.IndexOf("GALVA") >= 0
                    || std.IndexOf("GALVA") >= 0
                )
                    return false;
