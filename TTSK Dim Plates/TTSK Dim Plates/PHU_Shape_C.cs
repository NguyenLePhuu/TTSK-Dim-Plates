...
namespace Tekla.Technology.Akit.UserScript
{
    public class ShapeCScript
    {
        ...
        // C-SHAPE HOLE FACE CLASSIFICATION:
        ...
        private const double C_HOLE_DIRECTION_TIE_TOL = 0.02;
        private const double C_HOLE_CENTER_SIDE_TOL = 0.5;
        ...
        // Thép C dùng reserve 300mm để chừa vùng DIM dọc.
        ...
        // DEBUG TẠM - kiểm tra lỗ bị mất ở bước nào.
        // Sau khi tìm được lỗi có thể đổi false.
        private const bool DEBUG_C_SHAPE_HOLE_COUNT = false;
        ...
        private static double CurrentMiddleVerticalDimOffset = DIM_TIER_SCALE_15_MIDDLE;
        private static int LastTopMaxDimTier = 1;
        private static int LastFrontMaxDimTier = 1;
        private static int LastBottomMaxDimTier = 1;
        ...
        private static ModelPart CurrentCShapeHolePartForLocalClassify = null;
        private static View CurrentCShapeTopViewForHoleClassify = null;
        private static View CurrentCShapeFrontViewForHoleClassify = null;
        private static bool CurrentCShapeHoleCatalogInitialized = false;
        private static readonly List<CShapeHoleRecord> CurrentCShapeHoleCatalog =
            new List<CShapeHoleRecord>();
        private static List<CShapeHoleCandidateCacheEntry> CShapeHoleCandidateCache =
            new List<CShapeHoleCandidateCacheEntry>();
        ...
        )
        {
            ...
            PendingAutoSectionSelectedPartId =
                selectedAssemblyPartId > 0 ? selectedAssemblyPartId : 0;
        }
        ...
        {
            HShapeAutoSectionPrecheckResult result = new HShapeAutoSectionPrecheckResult();

            TopBottomHoleCheckResult = -1;
            CurrentCShapeHolePartForLocalClassify = null;
            CurrentCShapeTopViewForHoleClassify = null;
            CurrentCShapeFrontViewForHoleClassify = null;
            CurrentCShapeHoleCatalogInitialized = false;
            CurrentCShapeHoleCatalog.Clear();
            CShapeHoleCandidateCache.Clear();

            try
            {
                if (drawing == null)
                {
                    result.Message = "Khong co drawing de precheck Auto Section Shape C.";
                    return result;
                }

                if (model == null || !model.GetConnectionStatus())
                {
                    result.Message = "Khong ket noi duoc model de precheck Auto Section Shape C.";
                    return result;
                }

                if (part == null || part.Identifier == null)
                {
                    result.Message = "Khong co part Shape C hop le de precheck Auto Section.";
                    return result;
                }
                ...
                if (views.Count == 0)
                {
                    result.Message = "Khong tim thay view chua Shape C de precheck Auto Section.";
                    return result;
                }
                ...
                bool hasAmbiguousSpecialSection =
                    result.SpecialTopSections.Count > 1 || result.SpecialBottomSections.Count > 1;
                ...
                bool hasCompleteLayoutForDrawing =
                    isSinglePartDrawing ? result.HasCompleteSingleLayout
                    : isAssemblyDrawing ? result.HasCompleteAssemblyLayout
                    : false;
                ...
                {
                    result.Message =
                        "Khong xac dinh duoc Top view Shape C de precheck Auto Section.";
                    return result;
                }

                if (frontViewByType == null)
                {
                    result.Message = "Khong xac dinh duoc FrontView Shape C bang ViewType.";
                    return result;
                }

                CurrentCShapeHolePartForLocalClassify = part;
                CurrentCShapeTopViewForHoleClassify = resolvedTopView;
                CurrentCShapeFrontViewForHoleClassify = frontViewByType;

                InitializeCShapeHoleCatalog(model, part, resolvedTopView, frontViewByType);
                ...
                result.HasTopBottomDifference = result.HoleResult == 1;
                result.IsValid = result.HoleResult == 0 || result.HoleResult == 1;

                if (!result.IsValid)
                {
                    result.Message = "Khong the hoan tat precheck lo Top/Bottom Shape C.";
                }
                else if (result.HasTopBottomDifference)
                {
                    result.Message = "Lo Top/Bottom Shape C khac nhau.";
                }
                else
                {
                    result.Message = "Lo Top/Bottom Shape C giong nhau.";
                }
            }
            catch (Exception ex)
            {
                result.Message = "Precheck Auto Section Shape C loi: " + ex.Message;
            }
        ...
        public static void Run(Tekla.Technology.Akit.IScript akit)
        {
            ...
            {
                part = TrySelectModelPart(model, new Identifier(autoSectionMainPartId));
            }
            ...
            {
                RecoverAutoSectionDimViewsForC(
            ...
            bool cShapeLongitudinalVertical =
                isAssemblyDrawing && IsCShapeLongitudinalVerticalInView(model, part, topViewByType);

            InitializeCShapeHoleCatalog(model, part, topViewByType, frontViewByType);
            ...
            List<View> dimViews = BuildDimViewsByViewTypeForC(
                topViewByType,
                frontViewByType,
                bottomViewByType,
                specialBottomSections
            ...
            View frontViewForTopBottomNotch = (dimViews.Count > 1) ? dimViews[1] : frontViewByType;
            ChamferInfluence frontNotchInfluenceForTopBottom = cShapeLongitudinalVertical
            ...
            )
            {
                ...
                CreateDimsForFrontView(model, part, backViewByType, true, out backBoundary);
                CommitAndWait(drawing, 250);
            ...
            List<View> bottomViews = FindManualBottomCandidateViews(
                dimViews,
                topView,
                frontView,
                smallestExactView
            ...
            bool beamGridDimensionsCreated = PHU_BeamGridDimensionEngine.CreatePreparedDimensions();
            if (beamGridDimensionsCreated)
            {
                LastTopMaxDimTier = Math.Max(
                    LastTopMaxDimTier,
                    PHU_BeamGridDimensionEngine.GetOutermostHorizontalTier(topView)
                );
                LastFrontMaxDimTier = Math.Max(
                    LastFrontMaxDimTier,
            ...
            if (!preserveVerticalUserLayout && !cShapeLongitudinalVertical)
            {
                ...
                for (int i = 0; i < bottomViews.Count; i++)
                {
                    ...
                    TopBoundary bottomBoundary =
                        (i < bottomBoundaries.Count) ? bottomBoundaries[i] : new TopBoundary();
            ...
            PHU_Slot09_DataCenterContext.RegisterFinalTopFrontArrangement(
                topView,
                frontView,
                cShapeLongitudinalVertical ? finalGreenBoxGap : finalTopFrontGreenBoxGap
            );
        ...
        private static void CheckTopBottomHolesAndMark(Model model, ModelPart part, View topView)
        {
            ...
            try
            {
                ...
                List<Point> topHoleVerticalPolygon = GetTopSectionFacePolygon(
                    solid,
                    solidMin,
                    solidMax
                );
                ...
                double holeVerticalMinX,
                    holeVerticalMaxX,
                    holeVerticalMinY,
                    holeVerticalMaxY;
                GetMinMax(
                    topHoleVerticalPolygon,
                    out holeVerticalMinX,
                    out holeVerticalMaxX,
                    out holeVerticalMinY,
                    out holeVerticalMaxY
                );
                ...
                List<HoleCheckInfo> topHoles = GetTopBottomCheckHolesFromView(
                    model,
                    topView,
                    minX,
                    maxX,
                ...
                List<HoleCheckInfo> bottomHoles = GetTopBottomCheckHolesFromView(
                    model,
                    topView,
                    minX,
                    maxX,
        ...
        )
        {
            ...
            try
            {
                double targetZ = (zMin + zMax) / 2.0;
                double flangeThickness = GetFlangeThicknessFromProfile(
                    CurrentCShapeHolePartForLocalClassify
                );
                ...
                CShapeHoleClassification classified = ClassifyCShapeHoleCandidates(
                    holes,
                    flangeThickness
                );
                List<CShapeHoleCandidate> selected = SelectCShapeCheckCandidatesByOldZHint(
                    classified.TopCandidates,
                    classified.BottomCandidates,
                    targetZ
                );

                foreach (CShapeHoleCandidate item in selected)
                {
                    ...
                    h.X = item.Point.X;
                    h.Y = item.Point.Y;
                    h.Diameter = item.HoleDiameter;
                    h.SlotX = item.SlotX;
                    h.SlotY = item.SlotY;
                    h.HoleType = item.HoleType;
                    h.Matched = false;
            ...
            result.Sort(
                delegate(HoleCheckInfo a, HoleCheckInfo b)
                {
        ...
        private static List<CShapeHoleCandidate> SelectCShapeCheckCandidatesByOldZHint(
            List<CShapeHoleCandidate> topCandidates,
            List<CShapeHoleCandidate> bottomCandidates,
            double targetZ
        )
        {
            if (topCandidates == null)
                topCandidates = new List<CShapeHoleCandidate>();
            if (bottomCandidates == null)
                bottomCandidates = new List<CShapeHoleCandidate>();

            double topZ = GetAverageCShapeCandidateViewZ(topCandidates);
            double bottomZ = GetAverageCShapeCandidateViewZ(bottomCandidates);

            if (topCandidates.Count == 0)
                return targetZ <= bottomZ ? bottomCandidates : new List<CShapeHoleCandidate>();
            if (bottomCandidates.Count == 0)
                return targetZ >= topZ ? topCandidates : new List<CShapeHoleCandidate>();
        ...
        private static double GetAverageCShapeCandidateViewZ(List<CShapeHoleCandidate> holes)
        {
            try
            {
                ...
                foreach (CShapeHoleCandidate item in holes)
                {
                    if (item == null || item.Point == null)
                        continue;
        ...
        )
        {
            ...
            foreach (HoleCheckInfo q in list)
            {
                ...
                if (Math.Abs(q.X - h.X) <= tol && Math.Abs(q.Y - h.Y) <= tol)
                {
        ...
        )
        {
            ...
            {
                if (topHoles == null)
                    topHoles = new List<HoleCheckInfo>();
                if (bottomHoles == null)
                    bottomHoles = new List<HoleCheckInfo>();
        ...
        )
        {
            ...
            try
            {
                ...
                bool isVerticalCMember =
                    isAssemblyDrawing && IsCShapeLongitudinalVertical(minX, maxX, minY, maxY);
                ...
                ChamferEdgeAnchors edgeAnchors = BuildChamferEdgeAnchors(
                    topPolygon,
                    minX,
                    maxX,
                    minY,
                ...
                {
                    ApplyTopBottomFrontNotchTotalAnchors(
                        ref edgeAnchors,
                        topFrontNotchChain,
                        topPolygon,
                        minX,
                ...
                DimOffsetAnchor4 offsetAnchors = BuildDimOffsetAnchor4(edgeAnchors);
                ...
                List<Point> topFlangeHoles = ConvertCShapeHoleCandidatesToDimPoints(
                ...
                // Ví dụ: nhóm phi lớn/cụm đại diện tầng 1, phi khác tiếp theo tầng 2, ...
                // DIM tổng luôn nằm ở tầng cuối cùng sau toàn bộ nhóm lỗ.
                int topReservedTier = topChamferTierReserved ? 1 : 0;
                int topHorizontalTier =
                    topReservedTier + (holeDimCreated ? (topHoleTierCount + 1) : 1);

                // DIM tổng dọc TOP VIEW:
            ...
            try
            {
                ...
                switch (scale)
                {
                    case 5:
                        ...
                        CurrentMiddleVerticalDimOffset = DIM_TIER_SCALE_5_MIDDLE;
                        break;

                    case 10:
                        CurrentDimTierBase = DIM_TIER_SCALE_10_BASE;
                        CurrentDimTierStep = DIM_TIER_SCALE_10_STEP;
                        CurrentMiddleVerticalDimOffset = DIM_TIER_SCALE_10_MIDDLE;
                        break;
                    ...
                    case 20:
                        ...
                        CurrentMiddleVerticalDimOffset = DIM_TIER_SCALE_20_MIDDLE;
                        break;

                    case 30:
                        CurrentDimTierBase = DIM_TIER_SCALE_30_BASE;
                        CurrentDimTierStep = DIM_TIER_SCALE_30_STEP;
                        CurrentMiddleVerticalDimOffset = DIM_TIER_SCALE_30_MIDDLE;
                        break;
                }
            }
            catch { }
        ...
        /// <summary>
        /// Exposes the C Shape tier contract after the final drawing scale has
        /// been resolved, for project-specific column add-on dimensions.
        /// </summary>
        public static bool TryGetCurrentColumnDimensionTierSpacing(
            out double tierBase,
            out double tierStep
        )
        {
            tierBase = CurrentDimTierBase;
            tierStep = CurrentDimTierStep;
            return !double.IsNaN(tierBase)
                && !double.IsInfinity(tierBase)
                && tierBase > 0.0
                && !double.IsNaN(tierStep)
                && !double.IsInfinity(tierStep)
                && tierStep > 0.0;
        }

        private static DimOffsetAnchor4 BuildDimOffsetAnchor4(ChamferEdgeAnchors edgeAnchors)
        {
        ...
        )
        {
            try
            {
                if (direction == null || anchors == null || !anchors.IsValid)
                    return tierOffset;
        ...
        )
        {
            ...
            return ResolveDimDistanceByAnchor4(dimPoints, direction, anchors, tierOffset);
        }
        ...
        {
            double distance = ResolveDimDistanceByAnchor4(p1, p2, direction, anchors, tierOffset);
            ...
            try
            {
                ...
                foreach (Point p in polygon)
                {
                    ...
                    if (
                        ...
                        || (
                            Math.Abs(p.X - anchors.LeftMost.X) <= edgeTol
                            && p.Y > anchors.LeftMost.Y
                        )
                    )
                    {
                    ...
                    if (
                        ...
                        || (
                            Math.Abs(p.X - anchors.RightMost.X) <= edgeTol
                            && p.Y > anchors.RightMost.Y
                        )
                    )
                    {
                    ...
                    if (
                        ...
                        || (
                            Math.Abs(p.Y - anchors.BottomMost.Y) <= edgeTol
                            && p.X < anchors.BottomMost.X
                        )
                    )
                    {
        ...
        )
        {
            ...
            try
            {
                ...
                {
                    pts.Add(Clone2D(chain.LeftOuter));
                ...
                {
                    double rightOuterY =
                        chain.LeftInner != null ? chain.LeftInner.Y : chain.LeftOuter.Y;
                    Point rightOuter = FindEdgePointNearestY(
                        polygon,
                        rightOuterY,
                        maxX,
                        true,
                ...
                {
                    double leftOuterY =
                        chain.RightInner != null ? chain.RightInner.Y : chain.RightOuter.Y;
                    Point leftOuter = FindEdgePointNearestY(
                        polygon,
                        leftOuterY,
                        minX,
                        true,
                ...
                double offset = ResolveDimDistanceByAnchor4(
                    pts,
                    new Vector(0, 1, 0),
                    offsetAnchors,
                    tierOffset
                ...
                if (handler.CreateDimensionSet(view, pts, new Vector(0, 1, 0), offset) != null)
                {
                    count++;
                    if (detectTopFrontNotch)
                        influence.Top = true;
                    else
        ...
        )
        {
            ...
            try
            {
                ...
                bool hasLeft = IsValidTopBottomFrontNotchSide(
                    leftOuter,
                    leftInner,
                    true,
                    minX,
                ...
                bool hasRight = IsValidTopBottomFrontNotchSide(
                    rightOuter,
                    rightInner,
                    false,
                    minX,
        ...
        private static bool IsPointOnRequestedCShapeFace(
            ModelPart part,
            Point point,
            double flangeThickness,
            bool wantTopFace
        )
        {
            try
            {
                ...
                double minHeight;
                double maxHeight;
                if (!TryGetCShapePartLocalYRange(part, out minHeight, out maxHeight))
                    return false;
                ...
                double ly;
                double lz;
                if (!TryGetCShapePartLocalPoint(part, point, out lx, out ly, out lz))
                    return false;

                return wantTopFace ? ly >= topMin : ly <= bottomMax;
            }
            catch
            {
                return false;
        ...
        )
        {
            ...
            return width >= minSize && depth >= minSize && width <= maxSize && depth <= maxSize;
        }
        ...
        )
        {
            ...
            try
            {
                ...
                List<Point> frontProjectedSolidPoints = GetProjectedSolidPointsForFrontNotchDims(
                    solid
                );
        ...
        )
        {
            ...
            foreach (Point p in pts)
            {
                ...
                {
                    if (outer == null || (topSide && p.Y > outer.Y) || (!topSide && p.Y < outer.Y))
                        outer = Clone2D(p);
                }
            ...
            try
            {
                ...
                if (bottomInner.Count >= 2)
                {
                    ...
                    if (
                        width >= minWidth
                        && depth >= minDepth
                        && width <= NOTCH_MAX_SIZE
                        && depth <= NOTCH_MAX_SIZE
                    )
                    {
                        ...
                        Point innerLeft = FindExtremePointOnHorizontalBand(
                            bottomInner,
                            y2,
                            true,
                            edgeTol,
                        ...
                        Point innerRight = FindExtremePointOnHorizontalBand(
                            bottomInner,
                            y2,
                            false,
                            edgeTol,
                        ...
                        if (innerLeft == null)
                            innerLeft = FindNearestPoint(bottomInner, x1, y2);
                        if (innerRight == null)
                            innerRight = FindNearestPoint(bottomInner, x2, y2);
                        ...
                        )
                        {
                            ...
                            bool useRightSideForDepth =
                                Math.Abs(maxX - notchMidX) < Math.Abs(notchMidX - minX);
                            ...
                            if (
                                CreateEdgeAnchoredNotchDimBySize(
                                    handler,
                                    view,
                                    Clone2D(depthOuter),
                                    Clone2D(depthInner),
                ...
                if (topInner.Count >= 2)
                {
                    ...
                    )
                    {
                        ...
                        if (innerLeft == null)
                            innerLeft = FindNearestPoint(topInner, x1, y1);
                        if (innerRight == null)
                            innerRight = FindNearestPoint(topInner, x2, y1);
                ...
                if (leftInner.Count >= 2)
                {
                    ...
                    )
                    {
                        ...
                        Point innerBottom = FindExtremePointOnVerticalBand(
                            leftInner,
                            x2,
                            true,
                            edgeTol,
                        ...
                        Point innerTop = FindExtremePointOnVerticalBand(
                            leftInner,
                            x2,
                            false,
                            edgeTol,
                        ...
                        if (innerBottom == null)
                            innerBottom = FindNearestPoint(leftInner, x2, y1);
                        if (innerTop == null)
                            innerTop = FindNearestPoint(leftInner, x2, y2);
                        ...
                        )
                        {
                            ...
                            bool useTopSideForDepth =
                                Math.Abs(maxY - notchMidY) < Math.Abs(notchMidY - minY);
                ...
                if (rightInner.Count >= 2)
                {
                    ...
                    )
                    {
                        ...
                        if (innerBottom == null)
                            innerBottom = FindNearestPoint(rightInner, x1, y1);
                        if (innerTop == null)
                            innerTop = FindNearestPoint(rightInner, x1, y2);
                ...
                if (hasTop && topNotch != null)
                {
                    if (
                        CreateEdgeAnchoredNotchDimBySize(
                            handler,
                            view,
                            Clone2D(topNotch.Outer),
                            Clone2D(topNotch.Inner),
                            new Vector(0, 1, 0),
                            horizontalOffset,
                            offsetAnchors,
                            topNotch.Width
                ...
                if (hasBottom && bottomNotch != null)
                {
                    if (
                        CreateEdgeAnchoredNotchDimBySize(
                            handler,
                            view,
                            Clone2D(bottomNotch.Outer),
                            Clone2D(bottomNotch.Inner),
                            new Vector(0, -1, 0),
                            horizontalOffset,
                            offsetAnchors,
                            bottomNotch.Width
            ...
            try
            {
                ...
                foreach (Point point in points)
                {
                    ...
                    if (Math.Abs(point.X - maxX) <= edgeTol)
                    {
                        ...
                        if (
                            inEndZone
                            && (outer == null || (isTop ? point.Y > outer.Y : point.Y < outer.Y))
                        )
                            outer = Clone2D(point);
                    }

                    if (Math.Abs(point.Y - endY) <= edgeTol && point.X < maxX - edgeTol)
                    {
                        double candidateWidth = maxX - point.X;
                        if (
                            candidateWidth >= minSize
                            && candidateWidth <= maxSize
        ...
        )
        {
            ...
            try
            {
                ...
                foreach (Point point in points)
                {
                    ...
                    if (Math.Abs(point.Y - endY) <= edgeTol && point.X > minX + edgeTol)
                    {
            ...
            try
            {
                ...
                if (isTopSide)
                {
                    ...
                    if (isLeftSide)
                    {
                        ...
                        return CreateRadiusDimByReflection(
                            view,
                            arc1,
                            arc2,
                            arc3,
            ...
            try
            {
                ...
                foreach (ConstructorInfo ctor in ctors)
                {
                    try
                    {
                        ...
                        if (
                            ps[0].ParameterType.IsAssignableFrom(view.GetType())
                        ...
                        )
                        {
                            ...
                            ps[0].ParameterType.IsAssignableFrom(typeof(Point))
                            && ps[1].ParameterType.IsAssignableFrom(typeof(Point))
                            && ps[2].ParameterType.IsAssignableFrom(typeof(Point))
        ...
        )
        {
            ...
            return TryGetVerticalNotchDepthSegment(
                pts,
                targetX,
                innerY,
                outerY,
        ...
        )
        {
            ...
            return TryGetHorizontalNotchDepthSegment(
                pts,
                targetY,
                innerX,
                outerX,
        ...
        )
        {
            ...
            TryCollectTopDepthFromEnumeratorMethod(
                obj,
                result,
                depth,
                "GetFaceEnumerator",
        ...
        )
        {
            try
            {
                ...
                while (enumerator.MoveNext())
                    CollectRealSolidPointsForTopDepth(
                        enumerator.Current,
                        result,
                        depth + 1,
                        zMin,
        ...
        )
        {
            ...
            try
            {
                ...
                Point segTop;
                if (
                    TryGetProjectedFrontNotchLongestVerticalSegmentNoFillet(
                        projectedPoints,
                        rightSide,
                        x1,
                        x2,
                ...
                if (
                    TryFindFrontNotchMaxFromOuterAnchorLine(
                        projectedPoints,
                        rightSide,
                        x1,
                        x2,
        ...
        )
        {
            ...
            try
            {
                ...
                double targetNotchY = useTopSideForDepth ? Math.Min(y1, y2) : Math.Max(y1, y2);
        ...
        )
        {
            ...
            try
            {
                ...
                foreach (Point seed in innerPoints)
                {
                    ...
                    if (
                        span > fallbackBestSpan + TOL
                        || (
                            Math.Abs(span - fallbackBestSpan) <= TOL
                            && sideScore < fallbackBestSideScore
                        )
                    )
                    {
        ...
        )
        {
            ...
            foreach (Point seed in pts)
            {
                ...
                double outerScore = GetDistanceToOuterEdge(
                    candidateEndpoint,
                    minX,
                    maxX,
                    minY,
        ...
        )
        {
            ...
            bool topLeftChamfer = (aOnLeft && bOnTop) || (bOnLeft && aOnTop);

            bool topRightChamfer = (aOnRight && bOnTop) || (bOnRight && aOnTop);

            bool bottomLeftChamfer = (aOnLeft && bOnBottom) || (bOnLeft && aOnBottom);

            bool bottomRightChamfer = (aOnRight && bOnBottom) || (bOnRight && aOnBottom);
        ...
        )
        {
            ...
            if (
                allowBeamGridHorizontalTakeover
        ...
        private static int CreateCShapeFrontHoleDimsLikeH(
            StraightDimensionSetHandler handler,
            View view,
            List<Point> holes,
            ...
            ChamferEdgeAnchors edgeAnchors,
            DimOffsetAnchor4 offsetAnchors,
            int firstHorizontalTier,
            int reservedHorizontalTier,
            int leftStartTier,
            int rightStartTier,
        ...
        )
        {
            ...
            try
            {
                if (holes == null || holes.Count == 0)
                {
                    ShowCShapeHoleDebugPopup(
                        view,
                        "Không có lỗ đầu vào sau bước lọc mặt.",
                        0,
                        null,
                        minX,
                        maxX
                ...
                Dictionary<double, List<Point>> groups =
                    technicalFamilies.Count > 0
                ...
                List<double> remainingKeys = new List<double>();
                foreach (double key in keys)
                {
                    if (HasCShapeHorizontalTwoEndClusters(groups[key]))
                        endPairKeys.Add(key);
                    else
                        remainingKeys.Add(key);
                }
                keys.Clear();
                keys.AddRange(endPairKeys);
                keys.AddRange(remainingKeys);

                if (DEBUG_C_SHAPE_HOLE_COUNT)
                {
                    ShowCShapeHoleDebugPopup(
                        view,
                        "Trước khi tạo DIM CHAIN",
                        holes.Count,
                        groups,
                        minX,
                        maxX
                ...
                List<List<Point>> orderedPhiMFamilies = BuildCShapePhiMPointFamiliesInKeyOrder(
                    technicalFamilies,
                    groups,
                    keys
                );
                ...
                foreach (List<Point> samePhiHoles in orderedPhiMFamilies)
                {
                    ...
                    int horizontalTier = firstHorizontalTier + horizontalUsedTierCount;
                    if (reservedHorizontalTier >= 0 && horizontalTier >= reservedHorizontalTier)
                        horizontalTier++;

                    double horizontalOffset = GetSteelDimOffsetByTier(horizontalTier);

                    int horizontalCreated = CreateCShapeDimChainHorizontal(
                        handler,
                        view,
                        samePhiHoles,
                        holes,
                    ...
                    if (horizontalCreated > 0)
                    {
                        ...
                        horizontalHighestTier = Math.Max(horizontalHighestTier, horizontalTier);
                    }

                    count += CreateCShapeFrontDimChainVerticalLikeH(
                        handler,
                        view,
                        samePhiHoles,
                        verticalPolygon,
        ...
        )
        {
            ...
            try
            {
                ...
                Dictionary<double, List<Point>> diameterGroups =
                    BuildCShapeDiameterGroupsFromTechnicalFamilies(technicalFamilies);
                List<double> diameterKeys = GetTopViewHoleDiameterKeysInTierOrder(diameterGroups);
                List<List<Point>> orderedPhiMFamilies = BuildCShapePhiMPointFamiliesInKeyOrder(
                    technicalFamilies,
                    diameterGroups,
                    diameterKeys
                ...
                foreach (List<Point> familyPoints in orderedPhiMFamilies)
                {
                    ...
                    highestHorizontalTier = Math.Max(highestHorizontalTier, localHorizontalHighest);
                }
            }
            catch { }
        ...
        private static int CreateCShapeAssemblyFrontNearestEdgeHoleDims(
            StraightDimensionSetHandler handler,
            View view,
            List<Point> holes,
            double minX,
        ...
        )
        {
            ...
            try
            {
                ...
                SplitCShapeAssemblyFrontHolesByX(
                    holes,
                    minX,
                    maxX,
                    out leftEndHoles,
                ...
                Point leftHorizontalHole = FindCShapeAssemblyFrontNearestEdgeHole(
                    leftEndHoles,
                    false
                );
                Point rightHorizontalHole = FindCShapeAssemblyFrontNearestEdgeHole(
                    rightEndHoles,
                    true
                );
                Point leftVerticalHole = FindCShapeAssemblyFrontVerticalRepresentativeHole(
                    leftEndHoles,
                    false
                );
                Point rightVerticalHole = FindCShapeAssemblyFrontVerticalRepresentativeHole(
                    rightEndHoles,
                    true
                );
                ...
                if (leftHorizontalHole != null)
                {
                    ...
                    horizontalDim.Add(
                        new Point(
                            leftHorizontalHole.X,
                            leftHorizontalHole.Y + GetHoleDimGap(leftHorizontalHole),
                            0
                        )
                    );
                ...
                if (leftVerticalHole != null)
                {
                    ...
                    double verticalOffset = GetSteelDimOffsetByTier(leftStartTier);
                    if (edgeAnchors.LeftMost != null)
                    {
                        verticalOffset = ResolveDimDistanceByAnchor4(
                            verticalDim,
                            new Vector(-1, 0, 0),
                            offsetAnchors,
                            verticalOffset
                ...
                if (rightHorizontalHole != null)
                {
                    ...
                    horizontalDim.Add(
                        new Point(
                            rightHorizontalHole.X,
                            rightHorizontalHole.Y + GetHoleDimGap(rightHorizontalHole),
                            0
                        )
                    );
                ...
                if (rightVerticalHole != null)
                {
                    ...
                    double verticalOffset = GetSteelDimOffsetByTier(rightStartTier);
        ...
        )
        {
            ...
            try
            {
                ...
                foreach (List<Point> samePhiHoles in orderedPhiMFamilies)
                {
                    ...
                    int horizontalTier = horizontalFirstTier + usedHorizontalTierCount;
                    if (horizontalReservedTier >= 0 && horizontalTier >= horizontalReservedTier)
                        horizontalTier++;

                    double horizontalOffset = GetSteelDimOffsetByTier(horizontalTier);
                    int horizontalCreated = 0;
                    int verticalTier = verticalFirstTier + usedVerticalTierCount;
                    if (verticalReservedTier >= 0 && verticalTier >= verticalReservedTier)
                        verticalTier++;
                    double verticalOffset = GetSteelDimOffsetByTier(verticalTier);
                    ...
                    foreach (List<Point> cluster in endClusters)
                    {
                        ...
                        Point representative = FindCShapeAssemblyVerticalMemberRepresentativeHole(
                            cluster,
                            placeBelow,
                            tol
                        );
                        ...
                        verticalCreated += CreateCShapeAssemblyVerticalMemberNearestEndDim(
                            handler,
                            view,
                            cluster,
                            isTopEnd,
                    ...
                    if (horizontalCreated > 0)
                    {
                        ...
                        highestHorizontalTier = Math.Max(highestHorizontalTier, horizontalTier);
                    }
                    ...
                    if (verticalCreated > 0)
                    {
                        ...
                        highestVerticalTier = Math.Max(highestVerticalTier, verticalTier);
                    }
                }
            }
            catch { }
        ...
        )
        {
            ...
            Point representative = FindCShapeRightmostHoleOnRow(cluster, representativeY, tol);
            return representative != null ? Clone2DWithDiameter(representative) : null;
        }
        ...
        )
        {
            try
            {
                ...
                double edgeReferenceY = GetCShapeVerticalMemberClusterEdgeReferenceY(
                    cluster,
                    minY,
                    maxY,
                    placeBelow
                ...
                double footY = placeBelow ? representative.Y - gap : representative.Y + gap;

                PointList pts = new PointList();
                pts.Add(Clone2D(sideEdge));
                pts.Add(new Point(representative.X, footY, 0));

                Vector direction = placeBelow ? new Vector(0, -1, 0) : new Vector(0, 1, 0);
                double realOffset = ResolveDimDistanceByAnchor4(
                    pts,
                    direction,
                    offsetAnchors,
                ...
                return handler.CreateDimensionSet(view, pts, direction, realOffset) != null ? 1 : 0;
            }
            catch
            {
                return 0;
        ...
        )
        {
            try
            {
                if (handler == null || view == null || endHoles == null || endHoles.Count == 0)
                    return 0;

                Point representative = FindCShapeAssemblyVerticalMemberNearestEndHole(
                    endHoles,
                    isTopEnd,
                    tol
                );
                ...
                double gap = GetCShapeClusterHoleDimGap(representative, endHoles);
                PointList pts = new PointList();
                pts.Add(Clone2D(edge));
                pts.Add(new Point(representative.X - gap, representative.Y, 0));
        ...
        )
        {
            ...
            Point representative = FindCShapeRightmostHoleOnRow(endHoles, representativeY, tol);
            return representative != null ? Clone2DWithDiameter(representative) : null;
        }
        ...
        )
        {
            ...
            try
            {
                ...
                if (clusters.Count == 1)
                {
                    double centerY =
                        (
                            ...
                            + GetCShapeMaxYFromPoints(clusters[0], maxY)
                        ) * 0.5;
                    List<Point> target =
                ...
                AddCShapeAssemblyFrontHoles(topEndHoles, clusters[clusters.Count - 1]);
            }
            catch { }
        }
        ...
        )
        {
            ...
            {
                double tol = Math.Max(2.0, TOL + 1.0);
                List<List<Point>> clusters = BuildCShapeDimChainClustersByX(
                    holes,
                    C_HORIZONTAL_HOLE_CLUSTER_GAP,
                    tol
                );
                ...
                if (clusters.Count == 1)
                {
                    double centerX = GetAverageX(clusters[0]);
                    List<Point> target =
                ...
                AddCShapeAssemblyFrontHoles(leftEndHoles, clusters[0]);
                AddCShapeAssemblyFrontHoles(rightEndHoles, clusters[clusters.Count - 1]);
            }
            catch { }
        }

        private static void AddCShapeAssemblyFrontHoles(List<Point> target, List<Point> source)
        {
            if (target == null || source == null)
                return;

            foreach (Point hole in source)
            {
                if (hole != null)
                    target.Add(Clone2DWithDiameter(hole));
            }
        }

        private static Point FindCShapeAssemblyFrontNearestEdgeHole(List<Point> holes, bool isRight)
        {
            Point best = null;
            ...
            try
            {
                ...
                foreach (Point hole in holes)
                {
                    ...
                    bool isCloserToSide = isRight ? hole.X > best.X + TOL : hole.X < best.X - TOL;
                    bool isSameColumnAndLower =
                        Math.Abs(hole.X - best.X) <= TOL && hole.Y < best.Y - TOL;

                    if (isCloserToSide || isSameColumnAndLower)
                        best = Clone2DWithDiameter(hole);
                }
            }
            catch
        ...
        private static Point FindCShapeAssemblyFrontVerticalRepresentativeHole(
            List<Point> holes,
            bool isRight
        )
        {
        ...
        private static int CreateCShapeFrontDimChainVerticalLikeH(
            StraightDimensionSetHandler handler,
            View view,
            List<Point> holes,
            List<Point> polygon,
            ...
            int leftStartTier,
            int rightStartTier,
            ref int usedLeftTierCount,
            ref int usedRightTierCount
        )
        {
            int count = 0;
            ...
            try
            {
                ...
                List<CShapeHoleGeometryFamily> geometryFamilies = BuildCShapeHoleGeometryFamilies(
                    holes,
                    tol
                );

                foreach (CShapeHoleGeometryFamily family in geometryFamilies)
                {
                    ...
                    foreach (List<Point> cluster in clusters)
                    {
                        ...
                        count += CreateCShapeFrontDimChainVerticalOneClusterLikeH(
                            handler,
                            view,
                            cluster,
                            polygon,
                ...
                if (geometryFamilies == null || geometryFamilies.Count == 0)
                {
                    count += CreateCShapeFrontDimChainVerticalOneClusterLikeH(
                        handler,
                        view,
        ...
        private static int CreateCShapeFrontDimChainVerticalOneClusterLikeH(
            StraightDimensionSetHandler handler,
            View view,
            List<Point> holes,
            List<Point> polygon,
        ...
        )
        {
            ...
            try
            {
                ...
                double partMinX = GetCShapeMinXFromPoints(polygon, holes[0].X);
                double partMaxX = GetCShapeMaxXFromPoints(polygon, holes[0].X);

                double clusterAvgX = GetAverageX(holes);
                double distToLeft = Math.Abs(clusterAvgX - partMinX);
                double distToRight = Math.Abs(partMaxX - clusterAvgX);

                bool nearLeft = distToLeft <= FRONT_END_HOLE_ZONE;
                bool nearRight = distToRight <= FRONT_END_HOLE_ZONE;
                bool isEdgeCluster = nearLeft || nearRight;
                bool useRightSide =
                    isEdgeCluster && nearRight && (!nearLeft || distToRight < distToLeft);
                ...
                foreach (Point r in rows)
                {
                    ...
                    Point refHole = FindCShapeFarthestHoleOnRowFromVerticalDim(
                        holes,
                        r.Y,
                        tol,
                        useRightSide
                ...
                if (isEdgeCluster)
                {
                    bottomEdge = FindCShapeRealContourPointOnHorizontalLine(
                        polygon,
                        minY,
                        !useRightSide,
                        partMinX,
                    ...
                    topEdge = FindCShapeRealContourPointOnHorizontalLine(
                        polygon,
                        maxY,
                        !useRightSide,
                        partMinX,
                ...
                {
                    double chainFootX = 999999999.0;
                    foreach (Point rf in rowFeet)
                    {
                        if (rf != null && rf.X < chainFootX)
                            chainFootX = rf.X;
                    }

                    if (chainFootX > 900000000.0)
                        chainFootX = holes[0].X;

                    bottomEdge = new Point(chainFootX, minY, 0);
                    topEdge = new Point(chainFootX, maxY, 0);
                    useRightSide = false;
                }
                ...
                Vector direction = useRightSide ? new Vector(1, 0, 0) : new Vector(-1, 0, 0);

                double verticalOffset;
                if (isEdgeCluster)
                {
                    int verticalTier = useRightSide
                        ? rightStartTier + usedRightTierCount
                        : leftStartTier + usedLeftTierCount;
                    verticalOffset = GetSteelDimOffsetByTier(verticalTier);
                }
                else
                {
                    verticalOffset = GetSteelDimOffsetByTier(1);
                }

                double realVerticalOffset = verticalOffset;
                if (isEdgeCluster)
                {
                    realVerticalOffset = ResolveDimDistanceByAnchor4(
                        pts,
                        direction,
                        offsetAnchors,
                        verticalOffset
                ...
                {
                    realVerticalOffset = GetCShapeMiddleVerticalDimOffsetCoveringCluster(
                        holes,
                        bottomEdge.X,
                        verticalOffset
                    );
                ...
                if (handler.CreateDimensionSet(view, pts, direction, realVerticalOffset) != null)
                {
                    count++;
        ...
        )
        {
            ...
            try
            {
                ...
                foreach (List<Point> samePhiHoles in orderedPhiMFamilies)
                {
                    ...
                    int horizontalTier = horizontalFirstTier + horizontalUsedTierCount;
                    if (horizontalReservedTier >= 0 && horizontalTier >= horizontalReservedTier)
                        horizontalTier++;

                    int horizontalCreated = CreateCShapeVerticalMemberHorizontalClusterDims(
                        handler,
                        view,
                        samePhiHoles,
                        polygon,
                    ...
                    int verticalTier = verticalFirstTier + verticalUsedTierCount;
                    if (verticalReservedTier >= 0 && verticalTier >= verticalReservedTier)
                        verticalTier++;
                    int verticalCreated = CreateCShapeVerticalMemberLongitudinalHoleDim(
                        handler,
                        view,
                        samePhiHoles,
                        edgeAnchors,
                    ...
                    if (verticalCreated > 0)
                    {
                        ...
                        verticalHighestTier = Math.Max(verticalHighestTier, verticalTier);
                    }
                }
            }
            catch { }
        ...
        )
        {
            try
            {
                ...
                List<Point> columns = BuildCShapeUniqueHoleColumnsForChain(holes, tol);
                ...
                bool placeBelow = ShouldPlaceVerticalMemberClusterDimBelow(holes, minY, maxY);
                ...
                foreach (Point column in columns)
                {
                    ...
                    Point refHole = FindCShapeVerticalMemberRepresentativeHole(
                        holes,
                        column.X,
                        tol,
                        placeBelow
                    ...
                    double gap = GetCShapeClusterHoleDimGap(refHole, holes);
                    double footY = placeBelow ? refHole.Y - gap : refHole.Y + gap;
                ...
                return handler.CreateDimensionSet(view, pts, direction, horizontalOffset) != null
                    ? 1
                    : 0;
            }
            catch
        ...
        )
        {
            try
            {
                ...
                {
                    if (row == null || row.Y <= bottomY + tol || row.Y >= topY - tol)
                        continue;

                    Point refHole = FindCShapeRightmostHoleOnRow(holes, row.Y, tol);
                    if (refHole == null)
                        refHole = row;
                ...
                return handler.CreateDimensionSet(view, pts, direction, realVerticalOffset) != null
                    ? 1
                    : 0;
            }
            catch
        ...
        )
        {
            ...
            try
            {
                ...
                for (int i = 1; i < ys.Count; i++)
                {
                    ...
                    {
                        AddCShapeHoleClusterByRowList(result, holes, currentYs, tol);
        ...
        private static double GetCShapeMinYFromPoints(List<Point> points, double fallback)
        {
        ...
        private static double GetCShapeMaxYFromPoints(List<Point> points, double fallback)
        {
        ...
        private static Point FindCShapeTopmostHoleInColumn(List<Point> holes, double x, double tol)
        {
            Point best = null;
        ...
        private static int CreateTopViewHoleDimsByDiameter(
            ...
            bool useHorizontalGeometryClusterRule,
            out int usedTierCount
        )
        {
            // THÉP C - DIM CHAIN TOÀN BỘ LỖ TOP / BOTTOM / FRONT.
            // DEBUG TẠM:
            // Mục tiêu là xem lỗ bị mất ở bước nào:
            // 1. holes đầu vào sau lọc mặt.
            // 2. group theo phi/M.
            // 3. cột X dùng cho DIM chain ngang.
            // 4. cụm X dùng cho DIM chain dọc.
            int count = 0;
            usedTierCount = 0;
            ...
            try
            {
                ...
                foreach (List<Point> samePhiHoles in orderedPhiMFamilies)
                {
                    ...
                    double horizontalOffset = GetSteelDimOffsetByTier(
                        reservedHorizontalTierCount + usedTierCount
                    );

                    // DIM ngang: giữ thuật toán hiện tại, chỉ debug số lỗ/cột trước khi dim.
                    if (useHorizontalGeometryClusterRule)
                    {
                        count += CreateCShapeDimChainHorizontal(
                            handler,
                            view,
                            samePhiHoles,
                            holes,
                    ...
                    {
                        count += CreateCShapeDimChainHorizontalLegacy(
                            handler,
                            view,
                            samePhiHoles,
                            minX,
                    ...
                    // DIM dọc: giữ thuật toán hiện tại.
                    // Offset cụm giữa dùng theo scale drawing, cùng hệ tầng DIM.
                    double verticalChainOffset = CurrentMiddleVerticalDimOffset;

                    count += CreateCShapeDimChainVertical(
                        handler,
                        view,
                        samePhiHoles,
                        verticalPolygon,
        ...
        private static void ShowCShapeHoleDebugPopup(
            View view,
            string titleText,
            int inputHoleCount,
            Dictionary<double, List<Point>> groups,
        ...
        )
        {
            ...
            try
            {
                ...
                sb.AppendLine("");
                sb.AppendLine("Input holes sau lọc mặt = " + inputHoleCount.ToString());
                sb.AppendLine("");

                if (groups == null || groups.Count == 0)
                {
                    sb.AppendLine("Không có group phi/M.");
                }
                else
                {
                    List<double> keys = GetTopViewHoleDiameterKeysInTierOrder(groups);
                    ...
                    foreach (double key in keys)
                    {
                        ...
                        if (samePhiHoles == null)
                            samePhiHoles = new List<Point>();

                        double tol = Math.Max(2.0, TOL + 1.0);

                        List<Point> columns = BuildCShapeUniqueHoleColumnsForChain(
                            samePhiHoles,
                            tol
                        );

                        List<List<Point>> verticalClusters = BuildCShapeDimChainClustersByX(
                            samePhiHoles,
                            200.0,
                            tol
                        );

                        double avgX = GetAverageX(samePhiHoles);
                        double distToLeft = Math.Abs(avgX - minX);
                        double distToRight = Math.Abs(maxX - avgX);

                        sb.AppendLine("Phi/M key = " + key.ToString("0.###"));
                        sb.AppendLine("  So tam lo trong group = " + samePhiHoles.Count.ToString());
                        sb.AppendLine(
                            "  So cot X dim ngang = "
                                + (columns == null ? 0 : columns.Count).ToString()
                        );
                        sb.AppendLine(
                            "  So cum doc split 200 = "
                                + (verticalClusters == null ? 0 : verticalClusters.Count).ToString()
                        );
                        sb.AppendLine("  AvgX = " + avgX.ToString("0.###"));
                        sb.AppendLine(
                            "  Dist left/right = "
                                + distToLeft.ToString("0.###")
                                + " / "
                                + distToRight.ToString("0.###")
                        );

                        if (samePhiHoles.Count > 0)
                        {
                            samePhiHoles.Sort(
                                delegate(Point a, Point b)
                                {
                            ...
                            sb.AppendLine("  Danh sach tam lo:");
                            foreach (Point h in samePhiHoles)
                            {
                                if (h == null)
                                    continue;
                                sb.AppendLine(
        ...
        private static bool IsTopViewRectangularHoleCluster(List<Point> holes)
        {
            // Chỉ dùng cho TOP VIEW để phân biệt lỗ đơn và cụm lỗ thật.
            // Nếu cùng một nhóm có >= 2 cột X và >= 2 hàng Y thì xem là cụm lỗ,
            // không dùng rule lỗ đơn theo phi cố định nữa.
            try
            {
                ...
                foreach (Point h in holes)
                {
                    ...
                    AddUniqueCoordinate(xs, h.X, tol);
                    AddUniqueCoordinate(ys, h.Y, tol);
        ...
        )
        {
            try
            {
                ...
                if (technicalFamilies.Count == 0)
                {
                    return HasTopViewRectangularHoleClusterOnSide(holes, minX, maxX, leftSide);
                }
                ...
                foreach (CShapeHoleTechnicalFamily family in technicalFamilies)
                {
                    if (family == null || family.Holes == null)
                        continue;
                    if (!IsPhi24DiameterKey(GetCShapeLegacyDiameterKey(family.HoleDiameter)))
                        continue;

                    List<Point> familyPoints = ConvertCShapeHoleCandidatesToDimPoints(family.Holes);
                    if (HasTopViewRectangularHoleClusterOnSide(familyPoints, minX, maxX, leftSide))
                        return true;
                }
            }
            catch { }
        ...
        {
            // Chỉ kiểm tra nhánh cụm lỗ hình học đang tạo DIM dọc chain ở Top View.
            // Không thay đổi thuật toán nhận/lọc lỗ, chỉ dùng để quyết định tầng DIM tổng dọc.
            try
            {
                if (holes == null || holes.Count == 0)
                    return false;
                ...
                double beamCenterX = (minX + maxX) / 2.0;

                foreach (double key in groups.Keys)
                {
                    if (!IsPhi24DiameterKey(key))
                        continue;
                    ...
                    List<Point> sideHoles = new List<Point>();
                    foreach (Point h in samePhiHoles)
                    {
                        if (h == null)
                            continue;
                    ...
                    if (IsTopViewRectangularHoleCluster(sideHoles))
                        return true;
                }
            }
            catch { }
        ...
        private static List<double> GetTopViewHoleDiameterKeysInTierOrder(
            Dictionary<double, List<Point>> groups
        )
        {
            ...
            try
            {
                ...
                List<double> normalKeys = new List<double>();
                ...
                foreach (double key in normalKeys)
                    result.Add(key);
            }
            catch { }
        ...
        private static Dictionary<double, List<Point>> GroupTopViewHolesByDiameter(
            List<Point> holes
        )
        {
        ...
        private static bool IsPhi24DiameterKey(double key)
        {
            return Math.Abs(key - 24.0) <= 0.5;
        }
        ...
        private class CShapeHoleGeometryFamily
        {
            public List<Point> Holes = new List<Point>();
            public List<double> YPattern = new List<double>();
        }

        private static int CreateCShapeDimChainHorizontal(
            StraightDimensionSetHandler handler,
            View view,
            List<Point> holes,
            List<Point> allHoles,
        ...
        )
        {
            ...
            try
            {
                ...
                double tol = Math.Max(2.0, TOL + 1.0);
                List<CShapeHoleGeometryFamily> families = BuildCShapeHoleGeometryFamilies(
                    holes,
                    tol
                );

                foreach (CShapeHoleGeometryFamily family in families)
                {
                    if (family == null || family.Holes == null || family.Holes.Count == 0)
                        continue;
                    ...
                    List<List<Point>> clusters = BuildCShapeDimChainClustersByX(
                        family.Holes,
                        C_HORIZONTAL_HOLE_CLUSTER_GAP + tol + 0.001,
                        tol
                    );
                    ...
                    clusters.Sort(
                        delegate(List<Point> a, List<Point> b)
                        {
                    ...
                    if (clusters.Count >= 3)
                    {
                        count += CreateCShapeDimChainHorizontalSegment(
                            handler,
                            view,
                            family.Holes,
                            minX,
                    ...
                    {
                        double centerX = GetAverageX(clusters[0]);
                        bool hasHoleOnLeft = false;
                        bool hasHoleOnRight = false;
                        double clusterMinX = 999999999.0;
                        double clusterMaxX = -999999999.0;

                        foreach (Point h in clusters[0])
                        {
                            if (h == null)
                                continue;
                            if (h.X < clusterMinX)
                        ...
                        if (allHoles != null)
                        {
                            foreach (Point h in allHoles)
                            {
                                if (h == null)
                                    continue;
                                if (h.X < clusterMinX - tol)
                                    hasHoleOnLeft = true;
                                if (h.X > clusterMaxX + tol)
                                    hasHoleOnRight = true;
                            }
                        }

                        bool isMiddleCluster = hasHoleOnLeft && hasHoleOnRight;
                        bool useLeftEdge =
                            isMiddleCluster || Math.Abs(centerX - minX) <= Math.Abs(maxX - centerX);
                        bool useRightEdge = isMiddleCluster || !useLeftEdge;
        ...
        private static List<CShapeHoleGeometryFamily> BuildCShapeHoleGeometryFamilies(
            List<Point> holes,
            double tol
        )
        {
            List<CShapeHoleGeometryFamily> result = new List<CShapeHoleGeometryFamily>();
            ...
            try
            {
                ...
                List<List<Point>> columns = new List<List<Point>>();
                ...
                foreach (Point h in sorted)
                {
                    ...
                    {
                        targetColumn = new List<Point>();
                        columns.Add(targetColumn);
                    }

                    targetColumn.Add(Clone2DWithDiameter(h));
                }
                ...
                foreach (List<Point> column in columns)
                {
                    ...
                    List<double> yPattern = new List<double>();
                    foreach (Point h in column)
                    {
                        if (h != null)
                            AddUniqueCoordinate(yPattern, h.Y, tol);
                    }
                    yPattern.Sort();

                    CShapeHoleGeometryFamily family = null;
                    foreach (CShapeHoleGeometryFamily candidate in result)
                    {
                        if (
                            candidate != null
                            && AreCShapeHoleYPatternsSame(candidate.YPattern, yPattern, tol)
                        )
                        {
                            family = candidate;
                            break;
                    ...
                    if (family == null)
                    {
                        family = new CShapeHoleGeometryFamily();
        ...
        private static bool AreCShapeHoleYPatternsSame(List<double> a, List<double> b, double tol)
        {
            try
            {
        ...
        private static bool HasCShapeHorizontalTwoEndClusters(List<Point> holes)
        {
            try
            {
                if (holes == null || holes.Count == 0)
                ...
                foreach (CShapeHoleGeometryFamily family in families)
                {
                    ...
                    if (clusters != null && clusters.Count == 2)
                        return true;
                }
            }
            catch { }
        ...
        private static int CreateCShapeDimChainHorizontalSegment(
            StraightDimensionSetHandler handler,
            View view,
            List<Point> holes,
            double minX,
        ...
        )
        {
            ...
            try
            {
                ...
                double topEdgeY = edgeAnchors.TopMost != null ? edgeAnchors.TopMost.Y : maxY;

                double leftEdgeX = edgeAnchors.LeftMost != null ? edgeAnchors.LeftMost.X : minX;

                double rightEdgeX = edgeAnchors.RightMost != null ? edgeAnchors.RightMost.X : maxX;

                Point leftEdge = new Point(leftEdgeX, topEdgeY, 0);
                Point rightEdge = new Point(rightEdgeX, topEdgeY, 0);
                ...
                bool rightEdgeOnly = !includeLeftEdge && includeRightEdge;
                if (rightEdgeOnly)
                {
                    columns.Sort(
                        delegate(Point a, Point b)
                ...
                foreach (Point c in columns)
                {
                    ...
                    Point refHole = FindCShapeBottommostHoleInColumn(holes, c.X, tol);
                    if (refHole == null)
                        refHole = c;
                ...
                if (includeRightEdge && !rightEdgeOnly)
                    pts.Add(Clone2D(rightEdge));

                if (pts.Count < 2)
                    return count;

                double realHorizontalOffset = ResolveDimDistanceByAnchor4(
                    pts,
                    new Vector(0, 1, 0),
                    offsetAnchors,
                    horizontalOffset
                ...
                if (
                    handler.CreateDimensionSet(view, pts, new Vector(0, 1, 0), realHorizontalOffset)
                    != null
                )
                {
                    count++;
        ...
        private static int CreateCShapeDimChainHorizontalLegacy(
            StraightDimensionSetHandler handler,
            View view,
            List<Point> holes,
            double minX,
        ...
        {
            // THÉP C - DIM CHAIN NGANG LIÊN TỤC:
            // - Giữ toàn bộ cột X thật của lỗ, không dùng X trung bình.
            // - Không tách cụm ngang nữa, để không mất kích thước nối giữa các cụm.
            // - Chain đầy đủ: Mép trái -> lỗ -> lỗ -> ... -> lỗ -> Mép phải.
            // - Như vậy các khoảng giữa cụm trái / cụm phải vẫn được tạo trong cùng một dãy DIM.
            int count = 0;
            ...
            try
            {
                ...
                Point leftEdge =
                    edgeAnchors.LeftMost != null
                        ? Clone2D(edgeAnchors.LeftMost)
                        : new Point(minX, topEdgeY, 0);

                Point rightEdge =
                    edgeAnchors.RightMost != null
                        ? Clone2D(edgeAnchors.RightMost)
                        : new Point(maxX, topEdgeY, 0);
                ...
                foreach (Point c in columns)
                {
                    ...
                    // Nếu lỗ sát mép trên thì chân DIM bám mép thật, tránh tạo chân giả quá sát mép.
                    if (topEdgeY - refHole.Y <= gap + tol)
                        footY = topEdgeY;
        ...
        private static int CreateCShapeDimChainVertical(
            StraightDimensionSetHandler handler,
            View view,
            List<Point> holes,
            List<Point> polygon,
        ...
        {
            // THÉP C - DIM CHAIN DỌC:
            // Rule chuẩn DIM CHAIN:
            // - Nhóm cùng phi/M đã được truyền vào đây.
            // - Cho phép cả nhóm 1 lỗ thật để không bỏ sót lỗ đơn.
            // - Tách cụm theo X: gap >= 200 tạo chain riêng, gap < 200 gộp chung.
            // - Sau khi tách, mỗi cụm dù còn 1 lỗ vẫn phải DIM,
            //   vì nó là 1 phần của nhóm cùng phi/M ban đầu, không phải lỗ đơn thật sự.
            // - Cụm gần mép trái/phải: 2 chân ngoài cùng bám mép thật / điểm neo DIM tổng.
            // - Cụm giữa: giữ quy luật direction trái + offset 200, không dịch X thủ công.
            int count = 0;
            ...
            try
            {
                ...
                foreach (CShapeHoleGeometryFamily family in geometryFamilies)
                {
                    ...
                    foreach (List<Point> cluster in clusters)
                    {
                        ...
                        count += CreateCShapeDimChainVerticalOneCluster(
                            handler,
                            view,
                            cluster,
                            polygon,
        ...
        private static int CreateCShapeDimChainVerticalOneCluster(
            StraightDimensionSetHandler handler,
            View view,
            List<Point> holes,
            List<Point> polygon,
        ...
        )
        {
            ...
            try
            {
                ...
                foreach (Point r in rows)
                {
                    ...
                    // DIM CHAIN DỌC - RULE CHỐT:
                    // Chân vào lỗ luôn neo tại tâm lỗ offset đúng bằng phi.
                    // Không được ép chân lỗ bám mép, kể cả khi lỗ gần mép.
                    // Hai chân ngoài cùng của chain mới bám mép thật / điểm neo dim tổng.
                    rowFeet.Add(new Point(footX, refHole.Y, 0));
                }
                ...
                if (isEdgeCluster)
                {
                    // Cụm gần mép: hai chân ngoài cùng phải bám mép thật
                    // đúng phía gần nhất, giống điểm neo DIM tổng.
                    bottomEdge = FindCShapeRealContourPointOnHorizontalLine(
                        polygon,
                        minY,
                        !useRightSide,
                ...
                {
                    // Cụm giữa: không kéo về mép.
                    // Dùng X chân lỗ thật gần bên trái của cụm, rồi Tekla tự đẩy DIM
                    // bằng direction trái + distance verticalOffset (= 200).
                    double chainFootX = 999999999.0;
                    foreach (Point rf in rowFeet)
                    {
                        if (rf != null && rf.X < chainFootX)
        ...
        private static List<List<Point>> BuildCShapeDimChainClustersByX(
            List<Point> holes,
            double splitGap,
            double tol
        )
        {
            ...
            try
            {
                ...
                for (int i = 1; i < xs.Count; i++)
                {
                    ...
                    {
                        AddCShapeHoleClusterByColumnList(result, holes, currentXs, tol);
        ...
        private static void AddCShapeHoleClusterByColumnList(
            List<List<Point>> result,
            List<Point> holes,
            List<double> xs,
            double tol
        )
        {
            try
            {
                if (result == null || holes == null || xs == null || xs.Count == 0)
                    return;
                ...
                foreach (Point h in holes)
                {
                    ...
                    foreach (double x in xs)
                    {
                        ...
                        {
                            cluster.Add(Clone2DWithDiameter(h));
                            break;
                        }
                    }
                }
        ...
        private static List<Point> BuildCShapeUniqueHoleColumnsForChain(
            List<Point> holes,
            double tol
        )
        {
        ...
        private static List<Point> BuildCShapeUniqueHoleRowsForChain(List<Point> holes, double tol)
        {
            List<Point> result = new List<Point>();
        ...
        private static Point FindCShapeBottommostHoleInColumn(
            List<Point> holes,
            double x,
            double tol
        )
        ...
        private static Point FindCShapeLeftmostHoleOnRow(List<Point> holes, double y, double tol)
        {
            Point best = null;
        ...
        private static Point FindCShapeRightmostHoleOnRow(List<Point> holes, double y, double tol)
        {
            Point best = null;
        ...
        private static Point FindCShapeFarthestHoleOnRowFromVerticalDim(
            List<Point> holes,
            double y,
            double tol,
            bool dimOnRight
        ...
        {
            return dimOnRight
                ? FindCShapeLeftmostHoleOnRow(holes, y, tol)
                : FindCShapeRightmostHoleOnRow(holes, y, tol);
        }

        private static double GetCShapeMiddleVerticalDimOffsetCoveringCluster(
            List<Point> holes,
        ...
        )
        {
            try
            {
                ...
                foreach (Point hole in holes)
                {
                    ...
                    double gap = GetCShapeClusterHoleDimGap(hole, holes);
                ...
                return coveredWidth > 0.0 ? currentTierOffset + coveredWidth : currentTierOffset;
            }
            catch
            {
                return currentTierOffset;
        ...
        private static double GetCShapeClusterHoleDimGap(Point h, List<Point> holes)
        {
        ...
        private static double GetCShapeMinXFromPoints(List<Point> pts, double fallback)
        {
            try
            {
                if (pts == null || pts.Count == 0)
        ...
        private static double GetCShapeMaxXFromPoints(List<Point> pts, double fallback)
        {
            try
            {
                if (pts == null || pts.Count == 0)
        ...
        private static Point FindCShapeRealContourPointOnHorizontalLine(
            List<Point> polygon,
            double y,
            bool takeLeft,
            double minX,
        ...
        )
        {
            try
            {
                ...
                Point hit = FindCShapeHorizontalContourIntersectionInPointOrder(
                    polygon,
                    y,
                    takeLeft,
                    minX,
                ...
                List<Point> sorted = SortPolygonPointsClockwise(polygon);
                hit = FindCShapeHorizontalContourIntersectionInPointOrder(
                    sorted,
                    y,
                    takeLeft,
                    minX,
        ...
        private static Point FindCShapeHorizontalContourIntersectionInPointOrder(
            List<Point> pts,
            double y,
            bool takeLeft,
            double minX,
        ...
        )
        {
            try
            {
                ...
                for (int i = 0; i < pts.Count; i++)
                {
                    ...
                    double minSegY = Math.Min(a.Y, b.Y);
                    double maxSegY = Math.Max(a.Y, b.Y);

                    if (y < minSegY - tol || y > maxSegY + tol)
                        continue;
        ...
        private class CShapeHoleRecord
        {
        ...
        private class CShapeHoleCandidate
        {
            public Point Point;
            ...
            public CShapeHoleFace Face;
            public int BoltGroupId;
            public bool HasPartLocal;
            public double PartLocalX;
            public double PartLocalY;
            public double PartLocalZ;
        }

        private class CShapeHoleCluster
        {
        ...
        private class CShapeHoleClassification
        {
        ...
        private class CShapeHoleCandidateCacheEntry
        {
            ...
            public double MinY;
            public double MaxY;
            public bool UseTopBottomRealHoleDiameter;
            public List<CShapeHoleCandidate> Holes;
        }

        private static void InitializeCShapeHoleCatalog(
            Model model,
            ModelPart part,
            View topView,
            View frontView
        ...
        {
            CurrentCShapeHoleCatalog.Clear();
            CurrentCShapeTopViewForHoleClassify = topView;
            CurrentCShapeFrontViewForHoleClassify = frontView;
            CurrentCShapeHoleCatalogInitialized = false;
            ...
            try
            {
                ...
                ModelPart globalPart = model.SelectModelObject(part.Identifier) as ModelPart;
                if (globalPart == null)
                    globalPart = part;
                ...
                Vector frontNormal;
                if (
                    !TryGetCShapeViewNormal(topView, out topNormal)
                    || !TryGetCShapeViewNormal(frontView, out frontNormal)
                )
                    return;
                ...
                if (!TryGetCShapeSolidProjectionRange(solid, topNormal, out topMin, out topMax))
                    return;
                if (
                    !TryGetCShapeSolidProjectionRange(
                        solid,
                        frontNormal,
                        out frontMin,
                        out frontMax
                ...
                AddCShapeDrawingBoltGroupsToCatalog(
                    model,
                    topView,
                    globalPart,
                    topNormal,
            ...
            {
                CurrentCShapeHoleCatalogInitialized = true;
                model.GetWorkPlaneHandler().SetCurrentTransformationPlane(oldPlane);
            }
        }

        private static void AddCShapeDrawingBoltGroupsToCatalog(
            Model model,
            View view,
            ModelPart mainPart,
            Vector topNormal,
        ...
        )
        {
            try
            {
                ...
                DrawingObjectEnumerator boltObjects = view.GetAllObjects(
                ...
                while (boltObjects.MoveNext())
                {
                    ...
                    ModelBoltGroup bg = model.SelectModelObject(id) as ModelBoltGroup;
                    if (bg == null || !CShapeBoltGroupReferencesPart(bg, mainPart))
                        continue;
        ...
        private static void AddCShapeBoltGroupToCatalog(
            ModelBoltGroup bg,
            ModelPart mainPart,
            Vector topNormal,
            Vector frontNormal,
        ...
        )
        {
            try
            {
                ...
                bool hasHoleDirection = TryGetCShapeBoltDirection(bg, mainPart, out holeDirection);

                double topBottomHoleDiameter = GetTopBottomRealHoleDiameterFromBoltGroup(bg);
                ...
                if (
                    topBottomHoleDiameter <= MIN_VALID_HOLE_DIM_GAP
                ...
                foreach (object obj in bg.BoltPositions)
                {
                    ...
                    if (!hasHoleDirection)
                    {
                        direction = GetCShapePositionFallbackDirection(
                            modelPoint,
                            topNormal,
                            frontNormal,
                            topMin,
                    ...
                    CShapeHoleRecord record = new CShapeHoleRecord();
                    record.BoltGroupId = boltGroupId;
                    record.ModelPoint = new Point(modelPoint.X, modelPoint.Y, modelPoint.Z);
                    record.Face = ClassifyCShapeHoleFace(
                        bg,
                        modelPoint,
                        direction,
                        topNormal,
                    ...
                    AddUniqueCShapeHoleRecord(record);
                    addedAnyPosition = true;
                }

                if (addedAnyPosition && boltGroupId != 0 && addedBoltGroupIds != null)
                    addedBoltGroupIds.Add(boltGroupId);
            }
            catch { }
        }

        private static void AddUniqueCShapeHoleRecord(CShapeHoleRecord record)
        {
            if (record == null || record.ModelPoint == null)
                return;

            foreach (CShapeHoleRecord existing in CurrentCShapeHoleCatalog)
            {
                if (existing == null || existing.ModelPoint == null)
                    continue;

                if (record.BoltGroupId != 0 && existing.BoltGroupId != record.BoltGroupId)
                    continue;
            ...
            CurrentCShapeHoleCatalog.Add(record);
        }

        private static CShapeHoleFace ClassifyCShapeHoleFace(
            ModelBoltGroup bg,
            Point modelPoint,
            Vector holeDirection,
            Vector topNormal,
        ...
        {
            double topAlignment = Math.Abs(DotCShapeVectors(holeDirection, topNormal));
            double frontAlignment = Math.Abs(DotCShapeVectors(holeDirection, frontNormal));
            bool isFlangeHole;

            if (Math.Abs(topAlignment - frontAlignment) > C_HOLE_DIRECTION_TIE_TOL)
            {
            ...
            else
            {
                ...
                double topCoordinate = DotCShapePointVector(modelPoint, topNormal);
                double frontCoordinate = DotCShapePointVector(modelPoint, frontNormal);
                double topDistance = GetCShapeNormalizedOuterSurfaceDistance(
                    topCoordinate,
                    topMin,
                    topMax
                );
                double frontDistance = GetCShapeNormalizedOuterSurfaceDistance(
                    frontCoordinate,
                    frontMin,
                    frontMax
                );
            ...
            if (!isFlangeHole)
                return CShapeHoleFace.Front;

            double topCenter = (topMin + topMax) / 2.0;
            double side = DotCShapePointVector(modelPoint, topNormal) - topCenter;

            if (Math.Abs(side) <= C_HOLE_CENTER_SIDE_TOL)
            {
                try
                {
                    CoordinateSystem boltCs = bg.GetCoordinateSystem();
                    if (boltCs != null && boltCs.Origin != null)
                        side = DotCShapePointVector(boltCs.Origin, topNormal) - topCenter;
                }
                catch { }
            }

            if (Math.Abs(side) <= C_HOLE_CENTER_SIDE_TOL)
                side = DotCShapeVectors(holeDirection, topNormal);

            return side >= 0.0 ? CShapeHoleFace.Top : CShapeHoleFace.Bottom;
        }

        private static Vector GetCShapePositionFallbackDirection(
            Point modelPoint,
            Vector topNormal,
            Vector frontNormal,
            double topMin,
        ...
        {
            double topDistance = GetCShapeNormalizedOuterSurfaceDistance(
                DotCShapePointVector(modelPoint, topNormal),
                topMin,
                topMax
            );
            double frontDistance = GetCShapeNormalizedOuterSurfaceDistance(
                DotCShapePointVector(modelPoint, frontNormal),
                frontMin,
                frontMax
            );
        ...
        private static bool TryGetCShapeBoltDirection(
            ModelBoltGroup bg,
            ModelPart mainPart,
        ...
        )
        {
            ...
            try
            {
                ...
                {
                    Vector normal = CrossCShapeVectors(cs.AxisX, cs.AxisY);
                    if (TryNormalizeCShapeVector(normal, out direction))
                        return true;
                }
            }
            catch { }

            return TryGetCShapeConnectedPartDirection(bg, mainPart, out direction);
        }

        private static bool TryGetCShapeConnectedPartDirection(
            ModelBoltGroup bg,
            ModelPart mainPart,
            out Vector direction
        )
        {
            ...
            try
            {
                ...
                Point mainCenter;
                if (!TryGetCShapePartSolidCenter(mainPart, out mainCenter))
                    return false;
                ...
                foreach (ModelPart connectedPart in connectedParts)
                {
                    ...
                    Point center;
                    if (!TryGetCShapePartSolidCenter(connectedPart, out center))
                        continue;
                    ...
                    double length = GetCShapeVectorLength(delta);
                    if (length <= bestLength)
                        continue;

                    Vector normalized;
                    if (!TryNormalizeCShapeVector(delta, out normalized))
                        continue;
        ...
        private static bool TryGetCShapePartSolidCenter(ModelPart part, out Point center)
        {
            center = null;
        ...
        private static bool CShapeBoltGroupReferencesPart(ModelBoltGroup bg, ModelPart part)
        {
            try
            {
        ...
        private static bool TryGetCShapeViewNormal(View view, out Vector normal)
        {
            ...
            try
            {
                ...
                return TryNormalizeCShapeVector(CrossCShapeVectors(cs.AxisX, cs.AxisY), out normal);
            }
            catch
            {
                return false;
        ...
        private static bool TryGetCShapeSolidProjectionRange(
            Solid solid,
            Vector axis,
            out double minValue,
            out double maxValue
        )
        {
            ...
            try
            {
                    ...
                    while (faces.MoveNext())
                    {
                        ...
                        while (loops.MoveNext())
                        {
                            ...
                            while (vertices.MoveNext())
                            {
                                ...
                                double value = DotCShapePointVector(point, axis);
                                if (!hasValue)
                                {
                                    minValue = value;
                                    maxValue = value;
                ...
                if (readAllSolidFaces && hasValue && Math.Abs(maxValue - minValue) > 0.001)
                    return true;
                ...
                foreach (double x in xs)
                {
                    foreach (double y in ys)
                    {
                        ...
                        {
                            double value = DotCShapePointVector(new Point(x, y, z), axis);
                            if (!hasValue)
                            {
                                minValue = value;
                                maxValue = value;
        ...
        private static double GetCShapeNormalizedOuterSurfaceDistance(
            double coordinate,
            double minValue,
            double maxValue
        )
        {
            ...
            return Math.Min(Math.Abs(coordinate - minValue), Math.Abs(coordinate - maxValue))
                / range;
        }

        private static Vector CrossCShapeVectors(Vector a, Vector b)
        {
            if (a == null || b == null)
        ...
        private static double DotCShapeVectors(Vector a, Vector b)
        {
            if (a == null || b == null)
                return 0.0;
        ...
        private static double DotCShapePointVector(Point point, Vector vector)
        {
        ...
        private static double GetCShapeVectorLength(Vector vector)
        {
            if (vector == null)
                return 0.0;
        ...
        private static bool TryNormalizeCShapeVector(Vector vector, out Vector normalized)
        {
            normalized = new Vector(0, 0, 0);
            double length = GetCShapeVectorLength(vector);
            if (length <= 0.000001)
                return false;

            normalized = new Vector(vector.X / length, vector.Y / length, vector.Z / length);
            return true;
        }

        private static bool TryGetCShapePartLocalPoint(
            ModelPart part,
            Point modelPoint,
            out double lx,
            out double ly,
        ...
        )
        {
            ...
            try
            {
                ...
                NormalizeVector(ref axx, ref axy, ref axz);
                NormalizeVector(ref ayx, ref ayy, ref ayz);

                double azx = axy * ayz - axz * ayy;
                double azy = axz * ayx - axx * ayz;
                double azz = axx * ayy - axy * ayx;
                NormalizeVector(ref azx, ref azy, ref azz);
        ...
        private static void NormalizeVector(ref double x, ref double y, ref double z)
        {
        ...
        private static bool TryGetCShapePartLocalYRange(
            ModelPart part,
        ...
        )
        {
            ...
            try
            {
                ...
                if (!TryGetCShapePartLocalPoint(part, solid.MinimumPoint, out x1, out y1, out z1))
                    return false;

                if (!TryGetCShapePartLocalPoint(part, solid.MaximumPoint, out x2, out y2, out z2))
                    return false;

                minLocalY = Math.Min(y1, y2);
                maxLocalY = Math.Max(y1, y2);

                return Math.Abs(maxLocalY - minLocalY) > 1.0;
            }
            catch
            {
                return false;
        ...
        private static void TrySetCShapeCandidatePartLocalCoordinates(
            CShapeHoleCandidate item,
            Point modelPoint
        )
        {
            try
            {
                if (item == null || modelPoint == null)
                    return;
        ...
        private static List<CShapeHoleCandidate> FilterCShapeFlangeHoleCandidatesByPartLocalY(
            List<CShapeHoleCandidate> holes
        )
        {
            List<CShapeHoleCandidate> result = new List<CShapeHoleCandidate>();
            ...
            try
            {
                ...
                double midLocalY = (minLocalY + maxLocalY) / 2.0;

                foreach (CShapeHoleCandidate item in holes)
                {
                    if (item == null || !item.HasPartLocal)
                        continue;

                    double distToOuterFace = Math.Min(
                        Math.Abs(item.PartLocalY - minLocalY),
                        Math.Abs(item.PartLocalY - maxLocalY)
                    );

                    double distToWebMiddle = Math.Abs(item.PartLocalY - midLocalY);

                    // TOP/BOTTOM chỉ được nhận lỗ nằm về phía hai cánh.
                    // Nếu toàn bộ lỗ đang nằm ở bụng/web như localY gần tâm,
                    // trả rỗng để không dim nhầm lên Top/Bottom.
                    if (distToOuterFace < distToWebMiddle)
                        result.Add(item);
                }
            }
            catch { }
        ...
        private static bool TryGetCShapePartLocalZRange(
            ModelPart part,
        ...
        )
        {
            ...
            try
            {
                ...
                minLocalZ = Math.Min(z1, z2);
                maxLocalZ = Math.Max(z1, z2);

                return Math.Abs(maxLocalZ - minLocalZ) > 1.0;
            }
            catch
            {
                return false;
        ...
        private static double GetCShapeFaceZoneTolerance(double flangeThickness)
        {
            double tol = Math.Max(TOL * 3.0, 3.0);

            if (flangeThickness > 0.0 && flangeThickness < 100.0)
                tol = Math.Max(tol, flangeThickness + TOL * 2.0);

            return tol;
        }

        private static CShapeHoleClassification ClassifyCShapeHoleCandidates(
            List<CShapeHoleCandidate> holes,
            double flangeThickness
        )
        {
            CShapeHoleClassification result = new CShapeHoleClassification();
            ...
            try
            {
                ...
                foreach (CShapeHoleCandidate item in holes)
                {
                    ...
                    if (item.Face == CShapeHoleFace.Top)
                    {
                        result.TopCandidates.Add(item);
                    }
                    else if (item.Face == CShapeHoleFace.Bottom)
                    {
        ...
        private static List<CShapeHoleCandidate> FilterCShapeTopFlangeHoleCandidatesExactZone(
            List<CShapeHoleCandidate> holes,
            double flangeThickness
        )
        {
            ...
            try
            {
                ...
                double tol = GetCShapeFaceZoneTolerance(flangeThickness);
                double zoneMin = maxLocalY - tol;
                ...
                foreach (CShapeHoleCandidate item in holes)
                {
                    ...
                    // TOP chỉ nhận đúng vùng cánh trên theo PART-LOCAL Y.
                    // Không chọn nearest cluster nữa để tránh lấy nhầm Bottom/Web khi Top không có lỗ.
                    if (item.PartLocalY >= zoneMin && item.PartLocalY <= maxLocalY + tol)
                        result.Add(item);
                }
            }
            catch { }
        ...
        private static List<CShapeHoleCandidate> FilterCShapeBottomFlangeHoleCandidatesExactZone(
            List<CShapeHoleCandidate> holes,
            double flangeThickness
        )
        {
            ...
            try
            {
                ...
                foreach (CShapeHoleCandidate item in holes)
                {
                    ...
                    // BOTTOM chỉ nhận đúng vùng cánh dưới theo PART-LOCAL Y.
                    // Không chọn nearest cluster nữa để tránh lấy nhầm Top/Web khi Bottom không có lỗ.
                    if (item.PartLocalY >= minLocalY - tol && item.PartLocalY <= zoneMax)
                        result.Add(item);
                }
            }
            catch { }
        ...
        private static List<CShapeHoleCandidate> FilterCShapeWebHoleCandidatesExactZone(
            List<CShapeHoleCandidate> holes,
            double flangeThickness
        )
        {
            ...
            try
            {
                ...
                double minLocalZ;
                double maxLocalZ;
                bool hasLocalZRange = TryGetCShapePartLocalZRange(
                    CurrentCShapeHolePartForLocalClassify,
                    out minLocalZ,
                    out maxLocalZ
                );
                ...
                double tol = GetCShapeFaceZoneTolerance(flangeThickness);
                double innerMinY = minLocalY + tol;
                double innerMaxY = maxLocalY - tol;

                if (innerMinY > innerMaxY)
                {
                    double midY = (minLocalY + maxLocalY) / 2.0;
                    innerMinY = midY - tol;
                    innerMaxY = midY + tol;
                }

                // TEST FIX PHU 2026-07-05:
                // FRONT/WEB vẫn khóa đúng vùng bụng bằng PART-LOCAL Y,
                // nhưng chiều sâu PART-LOCAL Z cho phép rộng bằng full cánh/width.
                // Lý do: dump có lỗ Front thật Y=0 nhưng Z=-31/-28, trong khi solid depth chỉ -25..25.
                // Nếu dùng depthTol = flangeThickness/tol nhỏ sẽ làm rớt 2 lỗ mép trái.
                double depthTol = tol;
                if (hasLocalZRange)
                {
                    double fullFlangeDepth = Math.Abs(maxLocalZ - minLocalZ);
                    if (fullFlangeDepth > depthTol)
                        depthTol = fullFlangeDepth + TOL;
                }
                ...
                foreach (CShapeHoleCandidate item in holes)
                {
                    ...
                    // FRONT/WEB bắt buộc nằm giữa hai cánh theo PART-LOCAL Y.
                    // Lỗ cánh trên/dưới như Y = +/-50 sẽ bị loại ngay, không cho BuildCluster.
                    if (item.PartLocalY < innerMinY || item.PartLocalY > innerMaxY)
                        continue;

                    // Lọc thêm chiều sâu: web thật phải nằm sát một phía lưng C theo PART-LOCAL Z.
                    // Nếu không lấy được local Z range thì vẫn giữ điều kiện Y giữa hai cánh.
                    if (hasLocalZRange)
                    {
                        double distToDepthFace = Math.Min(
                            Math.Abs(item.PartLocalZ - minLocalZ),
                            Math.Abs(item.PartLocalZ - maxLocalZ)
                        );

                        if (distToDepthFace > depthTol)
                            continue;
                    }
        ...
        private static List<CShapeHoleCandidate> GetCShapeHoleCandidatesCachedInCurrentPlane(
            Model model,
            View view,
            double minX,
            double maxX,
        ...
        )
        {
            try
            {
                foreach (CShapeHoleCandidateCacheEntry entry in CShapeHoleCandidateCache)
                {
                    if (entry == null)
                        continue;

                    if (!IsSameViewForC(entry.View, view))
                        continue;

                    if (entry.UseTopBottomRealHoleDiameter != useTopBottomRealHoleDiameter)
                        continue;

                    if (
                        Math.Abs(entry.MinX - minX) > TOL
                        || Math.Abs(entry.MaxX - maxX) > TOL
                        || Math.Abs(entry.MinY - minY) > TOL
                        || Math.Abs(entry.MaxY - maxY) > TOL
                    )
                        continue;

                    if (entry.Holes != null)
                        return new List<CShapeHoleCandidate>(entry.Holes);

                    return new List<CShapeHoleCandidate>();
                }

                List<CShapeHoleCandidate> holes = GetCShapeHoleCandidatesInCurrentPlane(
                    model,
                    view,
                    minX,
                    maxX,
                    minY,
                    maxY,
                    useTopBottomRealHoleDiameter
                );

                CShapeHoleCandidateCacheEntry newEntry = new CShapeHoleCandidateCacheEntry();
                newEntry.View = view;
                newEntry.MinX = minX;
                newEntry.MaxX = maxX;
                newEntry.MinY = minY;
                newEntry.MaxY = maxY;
                newEntry.UseTopBottomRealHoleDiameter = useTopBottomRealHoleDiameter;
                newEntry.Holes = new List<CShapeHoleCandidate>(holes);
                CShapeHoleCandidateCache.Add(newEntry);

                return new List<CShapeHoleCandidate>(holes);
            }
            catch { }
        ...
        private static List<CShapeHoleCandidate> GetCShapeHoleCandidatesInCurrentPlane(
            Model model,
            View view,
            double minX,
            double maxX,
        ...
        )
        {
            ...
            {
                if (
                    (!CurrentCShapeHoleCatalogInitialized || CurrentCShapeHoleCatalog.Count == 0)
                    && CurrentCShapeHolePartForLocalClassify != null
                    && CurrentCShapeTopViewForHoleClassify != null
                    && CurrentCShapeFrontViewForHoleClassify != null
                )
                {
                    InitializeCShapeHoleCatalog(
                        model,
                        CurrentCShapeHolePartForLocalClassify,
                        CurrentCShapeTopViewForHoleClassify,
                        CurrentCShapeFrontViewForHoleClassify
                    );
                }
                ...
                Tekla.Structures.Geometry3d.Matrix toView = MatrixFactory.ToCoordinateSystem(
                    view.DisplayCoordinateSystem
                );
                Dictionary<int, double> drawingDiameterFallbacks =
                    GetCShapeDrawingHoleDiameterFallbacks(view);

                foreach (CShapeHoleRecord record in CurrentCShapeHoleCatalog)
                {
                    if (record == null || record.ModelPoint == null)
                        continue;
                    ...
                    if (
                        holeDiameter <= MIN_VALID_HOLE_DIM_GAP
                    ...
                    TrySetCShapeCandidatePartLocalCoordinates(item, record.ModelPoint);
        ...
        private static Dictionary<int, double> GetCShapeDrawingHoleDiameterFallbacks(View view)
        {
        ...
        private static List<CShapeHoleCluster> BuildCShapeHoleClusters(
            List<CShapeHoleCandidate> holes
        )
        {
            List<CShapeHoleCluster> clusters = new List<CShapeHoleCluster>();
            ...
            try
            {
                ...
                foreach (CShapeHoleCandidate item in holes)
                {
                    ...
                    double clusterTol = GetCShapeHoleClusterTolerance(item.HoleDiameter);
                    CShapeHoleCluster best = null;
                    double bestDist = 999999999.0;

                    foreach (CShapeHoleCluster cluster in clusters)
                    {
                        if (cluster == null)
                            continue;

                        double dy = item.Point.Y - cluster.CenterY;
                        double dz = item.Point.Z - cluster.CenterZ;
                        double dist = Math.Sqrt(dy * dy + dz * dz);

                        if (dist <= clusterTol && dist < bestDist)
                        {
                            bestDist = dist;
                            best = cluster;
                        }
                    ...
                    if (best == null)
                    {
                        best = new CShapeHoleCluster();
                        clusters.Add(best);
                    }

                    best.Holes.Add(item);
                    RecalculateCShapeHoleCluster(best);
                }
            }
            catch { }
        ...
        private static double GetCShapeHoleClusterTolerance(double holeDiameter)
        {
            double d = Math.Abs(holeDiameter);

            if (d <= MIN_VALID_HOLE_DIM_GAP || d > 200.0)
                d = Math.Max(TOL * 4.0, 4.0);

            return Math.Max(TOL * 4.0, d * 0.50);
        }

        private static void RecalculateCShapeHoleCluster(CShapeHoleCluster cluster)
        {
            if (cluster == null || cluster.Holes == null || cluster.Holes.Count == 0)
                return;
            ...
            foreach (CShapeHoleCandidate item in cluster.Holes)
            {
                if (item == null || item.Point == null)
                    continue;
                ...
                sy += item.Point.Y;
                sz += item.Point.Z;
                if (item.HoleDiameter > maxDia)
                    maxDia = item.HoleDiameter;
                count++;
            }

            if (count > 0)
            {
                cluster.CenterY = sy / count;
                cluster.CenterZ = sz / count;
                cluster.MaxHoleDiameter = maxDia;
            }
        }

        private static List<CShapeHoleCluster> SelectCShapeClustersNearestZ(
            List<CShapeHoleCluster> clusters,
            double targetZ
        )
        {
            List<CShapeHoleCluster> result = new List<CShapeHoleCluster>();
            ...
            try
            {
                ...
                foreach (CShapeHoleCluster cluster in clusters)
                {
                    ...
                    double dist = Math.Abs(cluster.CenterZ - targetZ);
                    if (dist < best)
                        best = dist;
                }
                ...
                foreach (CShapeHoleCluster cluster in clusters)
                {
                    ...
                    double clusterTol = GetCShapeHoleClusterTolerance(cluster.MaxHoleDiameter);
                    if (Math.Abs(cluster.CenterZ - targetZ) <= best + clusterTol)
                        result.Add(cluster);
                }
            }
            catch { }
        ...
        private static List<CShapeHoleCluster> SelectCShapeWebClustersByMiddleY(
            List<CShapeHoleCluster> clusters,
            double minY,
            double maxY
        )
        {
            ...
            try
            {
                ...
                foreach (CShapeHoleCluster cluster in clusters)
                {
                    ...
                    double dist = Math.Abs(cluster.CenterY - midY);
                    if (dist < best)
                        best = dist;
                }
                ...
                foreach (CShapeHoleCluster cluster in clusters)
                {
                    ...
                    double clusterTol = GetCShapeHoleClusterTolerance(cluster.MaxHoleDiameter);
                    if (Math.Abs(cluster.CenterY - midY) <= best + clusterTol)
                        result.Add(cluster);
                }
            }
            catch { }
            ...
            try
            {
                ...
                foreach (CShapeHoleCandidate hole in holes)
                {
                    ...
                    foreach (CShapeHoleTechnicalFamily family in result)
                    {
                        ...
                        if (
                            AreSameCShapeHoleTechnicalValue(family.HoleDiameter, hole.HoleDiameter)
                            && AreSameCShapeHoleTechnicalValue(family.BoltSize, hole.BoltSize)
                        )
                        {
                            found = family;
                            break;
        ...
        )
        {
            ...
            return ConvertCShapeClustersToDimPoints(BuildCShapeHoleClusters(holes));
        }

        private static Dictionary<
            double,
            List<Point>
        > BuildCShapeDiameterGroupsFromTechnicalFamilies(List<CShapeHoleTechnicalFamily> families)
        {
            Dictionary<double, List<Point>> result = new Dictionary<double, List<Point>>();
            ...
            try
            {
                ...
                foreach (CShapeHoleTechnicalFamily family in families)
                {
                    ...
                    foreach (Point point in familyPoints)
                    {
                        if (point == null)
                            continue;
                        AddUniquePoint(result[key], Clone2DWithDiameter(point), Math.Max(0.1, TOL));
                    }
                }
            }
            catch { }
        ...
        )
        {
            ...
            try
            {
                List<CShapeHoleTechnicalFamily> matched = new List<CShapeHoleTechnicalFamily>();
                ...
                if (technicalFamilies != null)
                {
                    foreach (CShapeHoleTechnicalFamily family in technicalFamilies)
                    {
                        if (family == null || family.Holes == null)
                            continue;
                        if (
                            Math.Abs(GetCShapeLegacyDiameterKey(family.HoleDiameter) - diameterKey)
                            > 0.001
                        )
                            continue;
                        matched.Add(family);
                ...
                matched.Sort(
                    delegate(CShapeHoleTechnicalFamily a, CShapeHoleTechnicalFamily b)
                    {
        ...
        )
        {
            ...
            try
            {
                ...
                foreach (double key in diameterKeys)
                {
                    ...
                    List<List<Point>> familiesForKey = GetCShapePhiMPointFamiliesForDiameterKey(
                        technicalFamilies,
                        key,
                        legacyFallback
                    );
        ...
        private static List<Point> ConvertCShapeClustersToDimPoints(
            List<CShapeHoleCluster> clusters
        )
        {
            List<Point> result = new List<Point>();
            ...
            try
            {
                ...
                foreach (CShapeHoleCluster cluster in clusters)
                {
                    if (cluster == null || cluster.Holes == null)
                        continue;
        ...
        )
        {
            try
            {
                ...
                holes = classified.TopCandidates;
                if (holes == null || holes.Count == 0)
                    return new List<Point>();

                List<CShapeHoleCluster> clusters = BuildCShapeHoleClusters(holes);

                return ConvertCShapeClustersToDimPoints(clusters);
            }
            catch { }
        ...
        )
        {
            try
            {
                ...
                holes = classified.BottomCandidates;
                if (holes == null || holes.Count == 0)
                    return new List<Point>();
        ...
        private static double GetTopBottomRealHoleDiameterFromBoltGroup(ModelBoltGroup bg)
        {
            try
            {
                ...
                // Không dùng rule riêng theo kích thước lỗ, không gán cứng đường kính.
                ...
                // Đây không phải rule cố định; nếu model khai tolerance/clearance thì phi lỗ
                // sẽ tự ra đúng theo dữ liệu model của từng bolt group.
                ...
                double tolerance = ReadFirstValidDouble(
                    ...
                    new string[] { "HoleTolerance", "BoltHoleTolerance", "Tolerance", "Clearance" }
                );

                if (
                    boltSize > MIN_VALID_HOLE_DIM_GAP
                    && boltSize < 200.0
                    && tolerance > 0.0
                    && tolerance < 50.0
                )
                ...
                // tuyệt đối không tự gán theo bảng size cố định. Trường hợp này hiếm và vẫn tốt hơn ghim vào tâm lỗ.
        ...
        private static double GetHoleDimGap(Point h)
        {
            try
            {
                ...
                // Top/Bottom không còn mã hóa đặc biệt theo kích thước lỗ.
        ...
        private static double GetHoleDiameterFromBoltGroup(ModelBoltGroup bg)
        {
            try
            {
                ...
                // Đây là fallback cũ cho Front khi Tekla chỉ trả bolt size.
        ...
        private static double GetHoleDiameterFromDrawingBolt(DrawingObject drawingBolt)
        {
            try
            {
                ...
                if (box == null)
                    box = TryInvokeNoArg(drawingBolt, "GetObjectAlignedBoundingBox");
                if (box == null)
                    box = TryInvokeNoArg(drawingBolt, "GetBoundingBox");
        ...
        private static Identifier TryGetModelIdentifier(DrawingObject drawingObject)
        {
            try
            {
                ...
                PropertyInfo prop = drawingObject
                    .GetType()
                    .GetProperty("ModelIdentifier", BindingFlags.Public | BindingFlags.Instance);
        ...
        private static bool IsBracketChannelProfile(ModelPart part)
        {
            try
            {
                if (part == null)
                ...
                if (string.IsNullOrWhiteSpace(profile))
                {
                    PropertyInfo profileProperty = part.GetType()
                        .GetProperty("Profile", BindingFlags.Public | BindingFlags.Instance);
                    object profileObject =
                        profileProperty != null ? profileProperty.GetValue(part, null) : null;
                    PropertyInfo profileStringProperty =
                        profileObject != null
                            ...
                            : null;
                    object value =
                        profileStringProperty != null
                            ? profileStringProperty.GetValue(profileObject, null)
                            : null;
                    profile = value != null ? value.ToString() : "";
                }

                return !string.IsNullOrWhiteSpace(profile) && profile.Trim().StartsWith("[");
            }
            catch
            {
                return false;
        ...
        private static List<Point> GetTopFacePolygon(Solid solid, Point min, Point max)
        {
            ...
            try
            {
                double topProjectedVisibleDepth = Math.Max(
                    0.0,
                    (max.Z - min.Z) - TOP_PROJECTED_BOTTOM_EXCLUDE
                );
        ...
        )
        {
            ...
            if (!string.IsNullOrEmpty(attributeName))
                dim = TryCreateDimensionSetWithAttributes(
                    handler,
                    view,
                    list,
                    direction,
            ...
            try
            {
                ...
                for (int i = 0; i < methods.Length; i++)
                {
                    ...
                    object result = method.Invoke(
                        handler,
                        new object[] { view, list, direction, distance, attr }
                    );
        ...
        private static void TryLoadAttributesObject(object attr, string attributeName)
        {
            try
            {
        ...
        private static View FindViewByViewTypeForC(
            List<View> views,
        ...
        )
        {
            try
            {
                ...
                foreach (View view in views)
                {
                    if (ViewTypeMatchesForC(view, exactViewTypeName, fallbackText))
                        return view;
                }
            }
            catch { }
        ...
        private static bool ViewTypeMatchesForC(
            View view,
            string exactViewTypeName,
            string fallbackText
        )
        ...
        private static void ClassifySectionViewsForC(
            List<View> views,
            View frontView,
            View topViewByType,
        ...
        )
        {
            try
            {
                ...
                foreach (View view in views)
                {
                    ...
                    if (!ViewTypeMatchesForC(view, "SectionView", "Section"))
                        continue;

                    // Nếu view này trùng Top/Bottom chuẩn thì không xử lý như Section thường.
                    if (
                        IsSameViewForC(view, topViewByType)
                        || IsSameViewForC(view, bottomViewByType)
                    )
                        continue;

                    bool isSpecial = false;

                    if (frontView != null && IsSectionWidthCloseToFrontForC(view, frontView))
                    {
                        if (view.Origin.Y > frontView.Origin.Y)
                        {
                            AddUniqueViewForC(specialTopSections, view);
                            isSpecial = true;
                        }
                        else if (view.Origin.Y < frontView.Origin.Y)
                        {
                            AddUniqueViewForC(specialBottomSections, view);
                            isSpecial = true;
                        }
                    }

                    if (!isSpecial)
                        AddUniqueViewForC(exactSectionViews, view);
                }
            }
            catch { }
        }

        private static void RecoverAutoSectionDimViewsForC(
            bool singlePartLayout,
            List<View> views,
            View frontView,
            View topViewByType,
        ...
        )
        {
            try
            {
                ...
                {
                    View topSection = FindAutoSectionCandidateForC(views, frontView, true);

                    if (topSection != null)
                    {
                        AddUniqueViewForC(specialTopSections, topSection);
                        RemoveViewForC(exactSectionViews, topSection);
                    }
                }

                if (
                    bottomViewByType == null
                    && specialBottomSections != null
                    && specialBottomSections.Count == 0
                )
                {
                    View bottomSection = FindAutoSectionCandidateForC(views, frontView, false);

                    if (bottomSection != null)
                    {
                        AddUniqueViewForC(specialBottomSections, bottomSection);
                        RemoveViewForC(exactSectionViews, bottomSection);
                    }
                }
            }
            catch { }
        ...
        private static View FindAutoSectionCandidateForC(
            List<View> views,
            View frontView,
            bool aboveFront
        )
        {
            ...
            try
            {
                ...
                double frontWidth = GetViewRestrictionBoxWidthForC(frontView);

                foreach (View view in views)
                {
                    if (view == null || !ViewTypeMatchesForC(view, "SectionView", "Section"))
                        continue;

                    double deltaY = view.Origin.Y - frontY;
                    if ((aboveFront && deltaY <= 0.5) || (!aboveFront && deltaY >= -0.5))
                        continue;

                    double verticalDistance = Math.Abs(deltaY);
                    double sectionWidth = GetViewRestrictionBoxWidthForC(view);
        ...
        private static void RemoveViewForC(List<View> views, View view)
        {
            ...
            {
                if (IsSameViewForC(views[i], view))
                    views.RemoveAt(i);
            }
        }

        private static bool IsSectionWidthCloseToFrontForC(View sectionView, View frontView)
        {
            try
            {
                double sectionWidth = GetViewRestrictionBoxWidthForC(sectionView);
                double frontWidth = GetViewRestrictionBoxWidthForC(frontView);
        ...
        private static double GetViewRestrictionBoxWidthForC(View view)
        {
            try
            {
        ...
        private static List<View> BuildDimViewsByViewTypeForC(
            View topView,
            View frontView,
            View bottomViewByType,
            List<View> specialBottomSections
        )
        {
            ...
            try
            {
                AddUniqueViewForC(result, topView);
                AddUniqueViewForC(result, frontView);
        ...
        private static bool IsSameViewForC(View a, View b)
        {
            if (a == null || b == null)
                return false;
        ...
        private static void AddUniqueViewForC(List<View> list, View view)
        {
            ...
            {
                if (IsSameViewForC(existing, view))
                    return;
            }
        ...
        private static void SetDrawingPartRepresentationExact(DrawingPart dp)
        {
            try
            {
                ...
                if (attrs != null)
                {
                    ...
                    try
                    {
                        PropertyInfo attrProp = dp.GetType()
        ...
        private static bool TrySetExactValueByPropertyType(object obj, PropertyInfo prop)
        {
            try
            {
        ...
        // - Thép C: requiredLength = chiều dài dầm theo topView + 300mm reserve.
        ...
        private static double GetBeamLengthInView(Model model, ModelPart part, View view)
        {
            TransformationPlane oldPlane = model
                .GetWorkPlaneHandler()
                .GetCurrentTransformationPlane();
        ...
        )
        {
            ...
            double requiredModelLength = beamLength + AUTO_SCALE_RESERVE;
        ...
        private static void SetViewScale(View view, double scale)
        {
            ...
            try
            {
                ...
                if (attrs != null)
                {
                    ...
                    try
                    {
                        PropertyInfo attrProp = view.GetType()
                            .GetProperty("Attributes", BindingFlags.Public | BindingFlags.Instance);
        ...
        )
        {
            try
            {
                ...
                double gap =
                    (GetSteelDimOffsetByTier(baseMaxTier) + GetSteelDimOffsetByTier(targetMaxTier))
                    * 1.0;
        ...
        )
        {
            try
            {
                ...
                if (
                    TryGetViewGreenPaperBoxForShape(frontView, out frontGreenBox)
                ...
                )
                {
                    ...
                    double greenDeltaX = frontGreenBox.MaxX + greenBoxGap - sectionGreenBox.MinX;
                    double greenDeltaY = greenFrontOrigin.Y - greenSectionOrigin.Y;

                    if (Math.Abs(greenDeltaX) > 0.01 || Math.Abs(greenDeltaY) > 0.01)
                    {
                        MoveViewBySheetDelta(sectionView, greenDeltaX, greenDeltaY);
                    }
                ...
                double gap = GetSteelDimOffsetByTier(LastFrontMaxDimTier) + 100.0 * shortScale;
        ...
        private static bool TryGetExactSectionGeometryBoundary(View view, out TopBoundary boundary)
        {
            boundary = new TopBoundary();
        ...
        private static bool TryGetDrawingPartGeometryBoundary(View view, out TopBoundary boundary)
        {
            ...
            try
            {
                ...
                while (parts.MoveNext())
                {
                    ...
                    if (box == null)
                        box = TryInvokeNoArg(partObj, "GetObjectAlignedBoundingBox");
                    if (box == null)
                        box = TryInvokeNoArg(partObj, "GetBoundingBox");
                    if (box == null)
                        continue;
        ...
        private static bool TryGetDrawingObjectPaperBoxForCenter(DrawingObject obj, out AABB box)
        {
            box = null;
        ...
        )
        {
            try
            {
                ...
                ApplyTopBottomSheetBlockLimitForCenter(
                    drawing,
                    sheetWidth,
                    sheetHeight,
                    margin,
                ...
                ApplyForcedTopBottomBlockLimitForCenter(
                    sheetWidth,
                    sheetHeight,
                    ref usableMinY,
                    ref usableMaxY
        ...
        )
        {
            try
            {
                ...
                foreach (ViewPaperBox b in boxes)
                {
                    if (b.MinY < currentMinY)
                        currentMinY = b.MinY;
                    if (b.MaxY > currentMaxY)
                        currentMaxY = b.MaxY;
                }
                ...
                for (int i = 0; i < boxes.Count; i++)
                {
                    ...
                    cursorMaxY = desiredMinY - nextGap;
                }
            }
            catch { }
        }
        ...
        )
        {
            try
            {
                ...
                foreach (ViewPaperBox box in leftBoxes)
                {
                    ...
                    double desiredCenterX = leftColumnMinX + leftWidth - box.Width * 0.5;
                ...
                for (int i = 0; i < rightBoxes.Count; i++)
                {
                    ...
                    double desiredCenterX = rightColumnMinX + box.Width * 0.5;
                    double desiredCenterY =
                        i == 0 && !double.IsNaN(firstLeftCenterY)
                            ? firstLeftCenterY
        ...
        )
        {
            try
            {
                ...
                List<VerticalViewAxisLayoutItem> leftItems = new List<VerticalViewAxisLayoutItem>();
                ...
                foreach (View view in leftViews)
                {
                    ...
                    if (
                        ...
                        || box.Width <= 1.0
                        || box.Height <= 1.0
                        || double.IsNaN(origin.X)
                        || double.IsInfinity(origin.X)
                        || double.IsNaN(origin.Y)
                        || double.IsInfinity(origin.Y)
                    )
                        return false;

                    leftItems.Add(
                        new VerticalViewAxisLayoutItem
                        {
                            View = view,
                            Box = box,
                            OriginX = origin.X,
                ...
                for (int row = 0; row < rowCount; row++)
                {
                    ...
                    {
                        rowAxisY[row] =
                            rowAxisY[row - 1] - rowLowerReach[row - 1] - gap - rowUpperReach[row];
                    }
                }
                ...
                for (int row = 0; row < leftItems.Count; row++)
                {
                    ...
                    desiredMinY = Math.Min(
                        desiredMinY,
                        rowAxisY[row] + item.Box.MinY - item.OriginY
                    );
                    desiredMaxY = Math.Max(
                        desiredMaxY,
                        rowAxisY[row] + item.Box.MaxY - item.OriginY
                    );
                }
                ...
                double shiftX = (currentMinX + currentMaxX - desiredMinX - desiredMaxX) * 0.5;
                double shiftY = (currentMinY + currentMaxY - desiredMinY - desiredMaxY) * 0.5;
        ...
        private static bool TryGetViewPurplePaperBoxForShape(View view, out ViewPaperBox box)
        {
            box = null;
        ...
        private static bool TryGetViewGreenPaperBoxForShape(View view, out ViewPaperBox box)
        {
            box = null;
        ...
        private static void UpdateDrawingTitle3Scale(Drawing drawing, View referenceView)
        {
            try
            {
                ...
                try
                {
                    ...
                    if (
                        attrProp != null
                        && attrProp.GetIndexParameters().Length == 0
                        && attrProp.CanRead
                    )
        ...
        {
            if (!AllowViewBoundaryResizeForCurrentDrawing || UseSelectedMainPartMode)
                return;
        ...
        )
        {
            ...
            try
            {
                ...
                ChamferEdgeAnchors frontEdgeAnchors = BuildChamferEdgeAnchors(
                    frontPolygon,
                    minX,
                    maxX,
                    minY,
                ...
                DimOffsetAnchor4 frontOffsetAnchors = BuildDimOffsetAnchor4(frontEdgeAnchors);
                ...
                // THÉP C - FRONT: lấy lỗ theo coordinate đang thấy trong view.
                // Không dùng mặt cắt. Chỉ lọc lỗ nằm trên BỤNG C:
                // lấy từ mép ngoài bụng C vào trong đúng 1 web thickness.
                // FRONT C - LỌC LỖ THEO CHIỀU DÀY 2 CÁNH XANH:
                ...
                List<Point> frontHoles = ConvertCShapeHoleCandidatesToDimPoints(
                    frontHoleCandidates
                );
                ...
                bool frontTopChamferDimCreated = frontChamferCount > 0 && frontChamferInfluence.Top;
                ...
                bool verticalCMemberHasFrontNotch = isVerticalCMember && frontNotchInfluence.Any;
                ...
                int frontEndHoleXTier = verticalCMemberHasFrontNotch
                    ? 1
                    : (frontNotchInfluence.Top ? 0 : (frontTopChamferDimCreated ? 1 : 0));
                int frontReservedNotchXTier = verticalCMemberHasFrontNotch
                    ? -1
                    : (frontNotchInfluence.Top ? 1 : -1);
                int frontReservedVerticalNotchTier = verticalCMemberHasFrontNotch ? 1 : -1;
                ...
                if (frontHoles != null && frontHoles.Count > 0)
                {
                    ...
                    {
                        holeCount += CreateCShapeFrontHoleDimsLikeH(
                            handler,
                            view,
                            frontHoles,
                            frontHoleCandidates,
                ...
                // - Tổng ngang phía trên nằm ngoài DIM/chamfer/rãnh phía trên.
                // - Tổng dọc bên trái nằm ngoài số DIM lỗ dọc trái thực tế đã tạo.
                // - Left/Right dùng bộ đếm riêng, không đẩy tầng chéo hướng.
                ...
                if (frontHorizontalHoleHighestTier >= 0)
                    frontHorizontalHighestTier = Math.Max(
                        frontHorizontalHighestTier,
                        frontHorizontalHoleHighestTier
                    );

                int frontHorizontalTotalTier = Math.Max(1, frontHorizontalHighestTier + 1);
                int frontVerticalTotalTier = isVerticalCMember
                    ? Math.Max(verticalCMemberHasFrontNotch ? 1 : 0, frontVerticalHoleHighestTier)
                        + 1
                    : leftStartTier + frontLeftVerticalHoleTierCount;
                int frontRightVerticalMaxTier =
                    frontRightVerticalHoleTierCount > 0
                        ? rightStartTier + frontRightVerticalHoleTierCount - 1
                        : rightStartTier;
                int frontRightVerticalTotalTier = rightStartTier + frontRightVerticalHoleTierCount;

                LastFrontMaxDimTier = Math.Max(
                    frontHorizontalTotalTier,
                    Math.Max(
                        frontVerticalTotalTier,
                        useAssemblyBracketRepresentativeRule
        ...
        // THÉP C - FRONT HOLE FILTER THEO 2 VÙNG CÁNH XANH
        // Chỉ sửa thuật toán XÉT LỖ Front, không đụng thuật toán DIM phía dưới.
        //
        // Ghi chú quan trọng:
        // - 2 vùng xanh trong hình là 2 cánh theo CHIỀU SÂU của Front view.
        // - Vì đang SetCurrentTransformationPlane(view.DisplayCoordinateSystem), tọa độ p.Z
        ...
        )
        {
            try
            {
                ...
                holes = classified.FrontCandidates;
                if (holes == null || holes.Count == 0)
                    return new List<Point>();
        ...
        )
        {
            ...
            // C guard giữ nguyên: chỉ Beam-horizontal flow mới được handoff Grid.
            // Report Top/Bottom thật trước khi hỏi Vertical takeover.
            bool reported = false;
            if (allowBeamGridHorizontalTakeover && PHU_BeamGridDimensionEngine.IsViewPrepared(view))
            {
            ...
            bool shouldTakeoverVertical =
                allowBeamGridHorizontalTakeover
                && PHU_BeamGridDimensionEngine.ShouldTakeOverVerticalTotal(view);
        ...
        private static int CreateCShapeAssemblyFrontRightTotalDim(
            StraightDimensionSetHandler handler,
            View view,
            ChamferEdgeAnchors edgeAnchors,
            DimOffsetAnchor4 offsetAnchors,
        ...
        )
        {
            ...
            try
            {
                ...
                ResizeViewBoundaryKeepDepth(view, min.X, max.X, min.Y, max.Y);
            }
            catch { }
            finally
            {
        ...
        private static ModelPart GetMainPartFromDrawing(Model model, Drawing drawing)
        {
            try
            {
        ...
        private static DrawingPart GetSelectedDrawingPart(DrawingHandler dh)
        {
            try
            {
                ...
                DrawingObjectEnumerator selected = dh.GetDrawingObjectSelector().GetSelected();
        ...
        private static bool IsBoltBelongsToSelectedMainPart(ModelBoltGroup bg, ModelPart mainPart)
        {
            try
            {
        ...
        private static ModelPart GetModelPartPropertyByReflection(object obj, string propertyName)
        {
            try
            {
        ...
        private static ModelPart TrySelectModelPart(Model model, Identifier identifier)
        {
            try
            {
        ...
        private static ModelPart FindLargestModelPartFromDrawingViews(Model model, Drawing drawing)
        {
        ...
        private static List<View> GetMainPartViews(Drawing drawing, Identifier mainPartIdentifier)
        {
            List<View> result = new List<View>();
        ...
        private static bool ViewContainsMainPart(View view, Identifier mainPartIdentifier)
        {
            try
            {
                    ...
                    try
                    {
                        ...
                        while (true)
                        {
                            ...
                            foreach (Type dimType in dimTypes)
                            {
                                try
                                {
                                    DrawingObjectEnumerator viewObjects = view.GetAllObjects(
                                        dimType
                                    );
        ...
        )
        {
            ...
            try
            {
                ...
                foreach (View view in views)
                {
                    ...
                    if (
                        System.Object.ReferenceEquals(view, topView)
        ...
        private static void CommitAndWait(Drawing drawing, int ms)
        {
            try
            {
