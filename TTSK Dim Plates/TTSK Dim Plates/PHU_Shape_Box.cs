...
{
    // Steel box / RHS / SHS dimension script.
    // The dimensioning, view arrangement and contour handling intentionally stay
    // identical to ShapeCScript. Only the hole-face geometry differs: a through
    // hole belongs to both opposite walls of a closed box section.
    public class ShapeBoxScript
    {
        ...
        // Dump 20260713_230526: khoảng đặt dấu góc theo mm trên giấy.
        private const double BOX_CHAMFER_ANGLE_DISTANCE_PAPER = 0.7324647523;
        private const double BOX_CHAMFER_ANGLE_RAY_MIN_LENGTH = 25.0;
        ...
        public static void Run(Tekla.Technology.Akit.IScript akit)
        {
            ...
            ChamferInfluence frontNotchInfluenceForTopBottom = DetectFrontNotchInfluenceOnly(
                model,
                part,
                frontViewForTopBottomNotch
            );
        ...
        private class BoxChamferSegment
        {
            // Điểm trên cạnh đứng của đầu tấm: origin của DIM góc và điểm giữa của chain.
            public Point SidePoint;

            // Điểm trên cạnh trên/dưới thực của tấm: đầu ngoài của cạnh vát.
        ...
        )
        {
            ...
            try
            {
                ...
                {
                    int chamferCount = CreateBoxChamferChainAndAngleDims(
                        handler,
                        view,
                        topPolygon,
                        minX,
                ...
                // TOP VIEW: nhận dạng rãnh trước, chưa tạo DIM.
                // A/B/C/D phải được chốt sau khi chân DIM tổng đã nhận rãnh hoàn tất.
                ...
                bool leftChamferTierReserved = chamferInfluence.Left;
                bool rightChamferTierReserved = chamferInfluence.Right;

                List<Point> topFlangeHoles = GetVisibleTopFlangeBoltCentersFromView(
                    model,
                    view,
                    minX,
                    maxX,
                ...
                {
                    int bottomHorizontalNotchTier = (bottomChamferTierReserved ? 1 : 0) + 1;
                    ...
                    notchCount = CreateAxisAlignedNotchDims(
                        ...
                        (leftChamferTierReserved ? 1 : 0) + 1,
                        (rightChamferTierReserved ? 1 : 0) + 1,
                        out notchInfluence,
                ...
                int leftVerticalTier = 1 + (leftChamferTierReserved ? 1 : 0) + leftNotchTierCount;
                if (leftClusterVerticalDimCreated)
                    leftVerticalTier++;

                int rightVerticalMaxTier =
                    (rightChamferTierReserved ? 1 : 0) + rightNotchTierCount + 1;

                LastTopMaxDimTier = Math.Max(
                    Math.Max(topHorizontalTier, leftVerticalTier),
                    rightVerticalMaxTier
                );
        ...
        )
        {
            ...
            try
            {
                ...
                List<Point> topFlangeHoles = GetVisibleBottomFlangeBoltCentersFromView(
                    model,
                    view,
                    minX,
                    maxX,
        ...
        )
        {
            ...
            try
            {
                ...
                if (
                    handler.CreateDimensionSet(view, pts, new Vector(0, 1, 0), realUpperOffset)
                    != null
                )
                {
                    count++;
        ...
        private static int CreateBoxChamferChainAndAngleDims(
            StraightDimensionSetHandler handler,
            View view,
            List<Point> polygon,
            double minX,
        ...
        )
        {
            ...
            try
            {
                ...
                List<BoxChamferSegment> chamfers = new List<BoxChamferSegment>();

                for (int i = 0; i < pts.Count; i++)
                {
                    Point a = pts[i];
                    Point b = pts[(i + 1) % pts.Count];
                    BoxChamferSegment chamfer;

                    if (!TryGetBoxChamferSegment(a, b, minX, maxX, minY, maxY, out chamfer))
                        continue;

                    if (ContainsSameBoxChamfer(chamfers, chamfer))
                        continue;

                    chamfers.Add(chamfer);
                    influence.Any = true;
                    influence.Left |= !chamfer.IsRight;
                    influence.Right |= chamfer.IsRight;
                    influence.Top |= chamfer.IsTop;
                    influence.Bottom |= !chamfer.IsTop;

                    if (CreateBoxChamferAngleDimension(view, chamfer))
                        count++;
                }

                double chainOffset = GetSteelDimOffsetByTier(0);

                if (
                    CreateBoxChamferSideChain(
                        handler,
                        view,
                        chamfers,
                        false,
        ...
        private static bool TryGetBoxChamferSegment(
            Point a,
            Point b,
            double minX,
            double maxX,
        ...
        )
        {
            ...
            // Không lọc cứng kích thước hay tỷ lệ: chỉ cần là cạnh xiên nối đúng hai mép thực.
            if (dx <= TOL || dy <= TOL)
                return false;
            ...
            if ((aOnLeft && bOnTop) || (bOnLeft && aOnTop))
                chamfer = MakeBoxChamfer(aOnLeft ? a : b, aOnTop ? a : b, false, true);
            else if ((aOnRight && bOnTop) || (bOnRight && aOnTop))
                chamfer = MakeBoxChamfer(aOnRight ? a : b, aOnTop ? a : b, true, true);
            else if ((aOnLeft && bOnBottom) || (bOnLeft && aOnBottom))
                chamfer = MakeBoxChamfer(aOnLeft ? a : b, aOnBottom ? a : b, false, false);
            else if ((aOnRight && bOnBottom) || (bOnRight && aOnBottom))
                chamfer = MakeBoxChamfer(aOnRight ? a : b, aOnBottom ? a : b, true, false);

            return chamfer != null;
        }

        private static BoxChamferSegment MakeBoxChamfer(
            Point sidePoint,
            Point outerPoint,
            bool isRight,
            bool isTop
        ...
        {
            return new BoxChamferSegment
            {
                SidePoint = Clone2D(sidePoint),
                OuterPoint = Clone2D(outerPoint),
                IsRight = isRight,
                IsTop = isTop
            };
        }

        private static bool ContainsSameBoxChamfer(
            List<BoxChamferSegment> chamfers,
            BoxChamferSegment candidate
        )
        {
            foreach (BoxChamferSegment item in chamfers)
            {
                if (
                    item.IsRight == candidate.IsRight
                    && item.IsTop == candidate.IsTop
                    && Distance2D(item.SidePoint, candidate.SidePoint) <= TOL
                    && Distance2D(item.OuterPoint, candidate.OuterPoint) <= TOL
                )
                    return true;
            }
        ...
        private static bool CreateBoxChamferAngleDimension(View view, BoxChamferSegment chamfer)
        {
            try
            {
                // Ba điểm logic theo đúng thứ tự người dùng chọn:
                // 1 = đầu chamfer trên cạnh đứng.
                // 2 = điểm thẳng đứng, cùng X và khác Y với điểm 1.
                // 3 = đầu chamfer trên mép ngang và là điểm neo.
                Point point1Origin = Clone2D(chamfer.SidePoint);
                Point point3Anchor = Clone2D(chamfer.OuterPoint);
                double rayLength = Math.Max(
                    BOX_CHAMFER_ANGLE_RAY_MIN_LENGTH,
                    Math.Abs(point1Origin.Y - point3Anchor.Y) * 4.0
                );
                Point point2Perpendicular = new Point(
                    point1Origin.X,
                    point1Origin.Y + (chamfer.IsTop ? rayLength : -rayLength),
                    0
                );
                ...
                // API dùng Point1 làm tia neo. Truyền 3 trước 2 để chân neo nằm tại điểm 3.
                // Góc đo vẫn là góc giữa tia 1-2 và tia 1-3 nên trị số không đổi.
                AngleDimension angle = new AngleDimension(
                    view,
                    point1Origin,
                    point3Anchor,
                    point2Perpendicular,
                    BOX_CHAMFER_ANGLE_DISTANCE_PAPER,
                    attributes
                );
        ...
        private static bool CreateBoxChamferSideChain(
            StraightDimensionSetHandler handler,
            View view,
            List<BoxChamferSegment> chamfers,
            bool isRight,
        ...
        {
            BoxChamferSegment bottom = null;
            BoxChamferSegment top = null;

            foreach (BoxChamferSegment chamfer in chamfers)
            {
                if (chamfer.IsRight != isRight)
                    continue;
            ...
            Point bottomOuter =
                bottom != null ? Clone2D(bottom.OuterPoint) : new Point(sideX, minY, 0);
            Point topOuter = top != null ? Clone2D(top.OuterPoint) : new Point(sideX, maxY, 0);

            PointList chain = new PointList();
            // Dump lưu dưới -> trên; kết quả là một chain liên tục giữa hai cạnh thực của tấm.
            chain.Add(bottomOuter);
            if (bottom != null)
                chain.Add(Clone2D(bottom.SidePoint));
            if (top != null)
                chain.Add(Clone2D(top.SidePoint));
            chain.Add(topOuter);

            Vector direction = isRight ? new Vector(1, 0, 0) : new Vector(-1, 0, 0);

            try
            {
                return handler.CreateDimensionSet(view, chain, direction, offset) != null;
            }
            catch
            {
                return false;
        ...
        )
        {
            ...
            if (PHU_BeamGridDimensionEngine.ShouldTakeOverHorizontalTotal(view))
            {
                createHorizontalTotal = !PHU_BeamGridDimensionEngine.ReportShapeHorizontalTotal(
                    ...
                    horizontalTotalTier,
                    GetSteelDimOffsetByTier(horizontalTotalTier),
                    ...
                    realLeftTotalOffset,
                    verticalTotalTier,
                    GetSteelDimOffsetByTier(verticalTotalTier),
        ...
        )
        {
            ...
            double fullSectionDepth = isFlangeHole
                ? Math.Abs(topMax - topMin)
                : Math.Abs(frontMax - frontMin);
            bool crossesOppositeWalls = IsBoxOppositeWallThroughHole(bg, fullSectionDepth);

            // A box-section through hole has two physical mouths, but only one
            // model centre. Keep that single centre and expose it to the relevant
            // pair of opposite drawing faces. This prevents doubled dimensions
            // while retaining the hole in both Top/Bottom views when appropriate.
            if (!isFlangeHole)
                return crossesOppositeWalls
                    ? CShapeHoleFace.FrontBackThrough
                    : CShapeHoleFace.Front;

            if (crossesOppositeWalls)
                return CShapeHoleFace.TopBottomThrough;
        ...
        private static bool IsBoxOppositeWallThroughHole(ModelBoltGroup bg, double fullSectionDepth)
        {
            try
            {
                if (bg == null || fullSectionDepth <= TOL)
                    return false;

                // Tekla's HOLE_TYPE_THROUGH means through one wall thickness.
                // It does not by itself mean that the bolt cut crosses the hollow
                // core and exits the opposite wall. First confirm the plain-hole
                // type, then compare CutLength with the full box depth in the
                // drilling direction.
                bool isPlainThrough = false;
                string[] reportNames = new string[]
                {
                    "PLAIN_HOLE_TYPE",
                    "HOLE_PLAIN_TYPE",
                    "BOLT_PLAIN_HOLE_TYPE"
                };

                foreach (string reportName in reportNames)
                {
                    string value = GetReportString(bg, reportName);
                    if (value.IndexOf("THROUGH", StringComparison.OrdinalIgnoreCase) >= 0)
                    {
                        isPlainThrough = true;
                        break;
                    }
                ...
                if (!isPlainThrough)
                {
                    string[] propertyNames = new string[] { "PlainHoleType", "HolePlainType" };

                    foreach (string propertyName in propertyNames)
                    {
                        string value = GetStringPropertyByReflection(bg, propertyName);
                        if (value.IndexOf("THROUGH", StringComparison.OrdinalIgnoreCase) >= 0)
                        {
                            isPlainThrough = true;
                            break;
                ...
                double cutLength = GetReportDouble(bg, "CUT_LENGTH");
                if (cutLength <= TOL)
                    cutLength = GetReportDouble(bg, "BOLT_CUT_LENGTH");
                if (cutLength <= TOL)
                    cutLength = GetDoublePropertyByReflection(bg, "CutLength");

                if (cutLength <= TOL)
                    return false;

                double spanTolerance = Math.Max(TOL * 2.0, 2.0);
                return cutLength + spanTolerance >= fullSectionDepth;
            }
            catch { }
        ...
        )
        {
            ...
            try
            {
                ...
                foreach (CShapeHoleCandidate item in holes)
                {
                    ...
                    else if (item.Face == CShapeHoleFace.TopBottomThrough)
                    {
                        // Same centre is deliberately available to both faces.
                        // Each view later projects and de-duplicates it independently.
                        result.TopCandidates.Add(item);
                        result.BottomCandidates.Add(item);
                    }
                    else
                    {
                        // FrontBackThrough is represented once in the Front view;
                        // the back mouth must not create a second dimension point.
        ...
        )
        {
            ...
            try
            {
                ...
                DimOffsetAnchor4 offsetAnchors = BuildDimOffsetAnchor4(frontEdgeAnchors);
                ...
                List<Point> frontHoles = GetVisibleFrontBoltCentersFromViewByCoordinate(
                    model,
                    view,
                    minX,
                    maxX,
                ...
                if (ENABLE_TOP_VIEW_CHAMFER_DIM)
                {
                    frontChamferCount = CreateBoxChamferChainAndAngleDims(
                        handler,
                        view,
                        frontPolygon,
                        minX,
                ...
                {
                    // THÉP C - FRONT: không còn CreateFrontEndHoleXDims / YDims / MiddleDims của I/H.
                    // Dùng lại logic DIM chain của Top cho toàn bộ lỗ mặt Front.
                    int holeCount = CreateTopViewHoleDimsByDiameter(
                        handler,
                        view,
                        frontHoles,
                        frontPolygon,
                        ...
                        offsetAnchors,
                        beamLength,
                        frontChamferInfluence.Top ? 1 : 0,
                        frontChamferInfluence.Bottom ? 1 : 0,
                        true,
                ...
                // FRONT C - DIM tổng nằm ngoài tầng lỗ/chamfer theo logic gần TOP.
                int frontHorizontalTotalTier =
                    (frontChamferInfluence.Top ? 1 : 0)
                    + (holeDimCreated ? (frontHoleTierCount + 1) : 1);

                int frontVerticalTotalTier = 1 + (frontChamferInfluence.Left ? 1 : 0);
        ...
        )
        {
            ...
            // Report Top/Bottom thật trước khi quyết định Vertical takeover.
            bool reported = false;
            if (PHU_BeamGridDimensionEngine.IsViewPrepared(view))
            {
            ...
            bool shouldTakeoverVertical = PHU_BeamGridDimensionEngine.ShouldTakeOverVerticalTotal(
                view
            );
        ...
        private static void DeleteAllDimensions(Drawing drawing)
        {
            ...
            // Chỉ quét đúng các loại DIM cần xóa. Shape Box tự tạo lại
            // AngleDimension chamfer nên phải xóa angle cũ để tránh nhân đôi.
            try
            {
                if (drawing == null)
                    return;
